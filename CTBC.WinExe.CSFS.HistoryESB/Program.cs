...
namespace CTBC.WinExe.CSFS.HistoryESB
{
    class Program : CommonBIZ
    {
        ...
        private void Process()
        {
            try
            {
                ...
                // 取得要發查的資料
                DataTable dt = GetCaseTrsRFDMSend();
        ...
        private string GenerateRequestXml(DataRow data)
        {
            ...
            string strCurrency = data["Currency"].ToString().Trim();
            ...
            try
            {
               ...
               // sb.AppendLine("<ns2:Curr>"+ strCurrency +"</ns2:Curr>");
        ...
        private bool AnalyticResponXMl(string strTrnNum, string strVersionNewID, string xmlString, string strQueryStatus)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(sRspCode) && (sRspCode == "0000" || sRspCode == "C001"))
                {
                    ...
                    if (sRspCode == "C001")
                    {
                        // 更新CaseTrsRFDMSend，為成功
                       EditCaseTrsRFDMSend(strTrnNum, "08", sRspCode, sRspMsg);
                    }
                    else
                    {
                        // 更新CaseTrsRFDMSend，為成功
                       EditCaseTrsRFDMSend(strTrnNum, "01", sRspCode, sRspMsg);
                    }
                ...
                else
                {
                    ...
                    // 更新CaseTrsRFDMSend，為失敗
                    EditCaseTrsRFDMSend(strTrnNum, "03", sRspCode, sRspMsg);

                    // 依據案件狀態
                    if (int.Parse(strQueryStatus) >= 6)
                    {
                        //  將version檔的RFDM發查狀態更新為【重查失敗】
                        EditCaseTrsQueryVersionErr(strVersionNewID, "08", sRspMsg);
                    }
                    else
                    {
                        //  將version檔的RFDM發查狀態更新為【失敗】
                        EditCaseTrsQueryVersionErr(strVersionNewID, "04", sRspMsg);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetCaseTrsRFDMSend()
        {
            string sqlSelect = @"SELECT distinct CaseTrsRFDMSend.TrnNum , CaseTrsRFDMSend.VersionNewID
      								  ,CASE WHEN len(CaseTrsRFDMSend.ID_NO) < 8 
                                        THEN BOPS000401Recv.CUST_ID_NO
                                        ELSE CaseTrsRFDMSend.ID_No
                                    END AS ID_NO ,
								  CASE WHEN  (CaseTrsRFDMSend.Curr <> 'TWD' and len(CaseTrsRFDMSend.Curr) =3 and substring(CaseTrsRFDMSend.Acct_No,6,2) <> '00' and len(CaseTrsRFDMSend.Acct_No) > 11) 
									    THEN  '00'+SUBSTRING(CaseTrsRFDMSend.Acct_No,6,12)+PARMCode.CodeNo
										ELSE CaseTrsRFDMSend.Acct_No
										END AS Acct_No  ,
	                                    CaseTrsQueryVersion.Currency,
                                        CONVERT(VARCHAR(8),CaseTrsRFDMSend.Start_Jnrst_Date ,112) AS  Start_Jnrst_Date,
                                        CONVERT(VARCHAR(8),CaseTrsRFDMSend.End_Jnrst_Date ,112) AS  End_Jnrst_Date,
                                        CaseTrsRFDMSend.Type ,
                                        CaseTrsRFDMSend.AcctDesc ,
                                        CaseTrsRFDMSend.RFDMSendStatus ,
                                        CaseTrsRFDMSend.RspCode ,
                                        CaseTrsRFDMSend.RspMsg ,
                                        CaseMaster.HistoryStatus AS QueryStatus,
                                        CaseTrsRFDMSend.FileName
                                    FROM    CaseTrsRFDMSend
                                    LEFT JOIN CaseTrsQueryVersion
	                                    ON CaseTrsRFDMSend.VersionNewID = CaseTrsQueryVersion.NewID
                                    LEFT JOIN CaseMaster
	                                    ON CaseTrsQueryVersion.CaseTrsNewID = CaseMaster.CaseId
									LEFT JOIN PARMcode 
									    ON CaseTrsRFDMSend.Curr = PARMCode.CodeDesc  and PARMCode.CodeType ='CaseCust_CURRENCY'  and len(CaseTrsRFDMSend.Acct_No) > 11
       LEFT JOIN BOPS000401Recv
	                                    ON CaseTrsRFDMSend.VersionNewID = BOPS000401Recv.VersionNewID 
                                       and BOPS000401Recv.Acct_No like '%'+substring(CaseTrsRFDMSend.Acct_No,6,12)+'%'  and  len(CaseTrsRFDMSend.ID_NO) < 8  WHERE   CaseTrsRFDMSend.RFDMSendStatus = '02' ";
        ...
        /// <returns></returns>
        public bool EditCaseTrsRFDMSend(string strTrnNum, string strRFDMSendStatus, string strRspCode, string strRspMsg)
        {
        ...
        /// <returns></returns>
        public bool EditCaseTrsQueryVersion(string strstrVersionNewID, string strRFDMSendStatus, string strRFDMQryMessage)
        {
        ...
        /// <param name="strRFDMQryMessage">RFDM狀態原因</param>
        /// <returns></returns>
        public bool EditCaseTrsQueryVersionErr(string strstrVersionNewID, string strStatus, string strRFDMQryMessage)
        {
            string sql = @"UPDATE [CaseTrsQueryVersion] 
                            SET 
                                [RFDMSendStatus] = '6',
                                ...
                                [ModifiedUser] = 'SYSTEM' 
                            WHERE NewID = @NewID;
 UPDATE CaseMaster SET HistoryStatus = @Status WHERE NewID IN (SELECT CaseTrsNewID FROM CaseTrsQueryVersion WHERE NewID = @NewID)
...
";
        ...
        /// <summary>
        /// 查詢CaseTrsRFDMSend檔，狀態是否都為01和08，若是返回true，否則返回false
        /// </summary>
        ...
        public bool QueryRFDMC001(string strVersionNewID)
        {
            ...
            string sqlSelect = @" 
                                ...
                                FROM CaseTrsRFDMSend 
                                WHERE VersionNewID = '" + strVersionNewID + @"' AND CaseTrsRFDMSend.RspCode = 'C001'
                                ";
