...
namespace CTBC.WinExe.CSFS.HistoryFile
{
    public class ExportReportPDF : BaseBusinessRule
    {
        ...
        /// <returns></returns>
        public DataTable Get401RecvData(string parCaseTrsNewID)
        {
              ...
              // 20180201,PeterHsieh:查無資料也要顯示
                string sqlSelect = @"
                                    select
	                                    ...
	                                    ,CaseTrsQueryVersion.CustId as CUST_ID_NO
	                                    ,CaseMaster.ROpenFileName
	                                    ,CaseMaster.DocNo
                                    	,CONVERT(varchar(100),CaseTrsQueryVersion.HTGModifiedDate, 111) as HTGQDateS --資料提供日期
	                                    ,CASE 
                                    ...
                                    left join
	                                    BOPS067050Recv
                                    on BOPS067050Recv.VersionNewID = CaseTrsQueryVersion.NewID
                                    and BOPS067050Recv.CUST_ID_NO = CaseTrsQueryVersion.CustId
                                    left join
	                                    BOPS000401Recv
                                    on BOPS000401Recv.VersionNewID = BOPS067050Recv.VersionNewID
                                    where 
	                                    CaseTrsQueryVersion.Status <> '99'
                                       and CaseTrsQueryVersion.HTGSendStatus = '99'
                                       and CaseTrsQueryVersion.OpenFlag = 'Y'
	                                    and CaseTrsQueryVersion.CaseTrsNewID = @CaseTrsNewID
                                    order by 
                ...
                base.Parameter.Add(new CommandParameter("@CaseTrsNewID", parCaseTrsNewID));
        ...
        /// <param name="pDocNo"></param>
        /// <returns></returns>
        public DataTable GetRFDMRecvData(string parCaseTrsNewID)
        {
            DataTable dtReturn = new DataTable();
                ...
                string sqlSelect = @"
                                    ...
                                    FROM (
	                                    select
		                                    ...
		                                    ,CaseMaster.DocNo
		                                    ,CASE
		                                    WHEN LEN(CaseTrsQueryVersion.QDateS) > 0 
			                                    and CaseTrsQueryVersion.QDateS <> ''
			                                    THEN CONVERT(NVARCHAR(10), CONVERT(DATETIME, CaseTrsQueryVersion.QDateS), 111) ELSE '' 
		                                    END AS QDateS
		                                    ,CASE
			                                    WHEN LEN(CaseTrsQueryVersion.QDateE) > 0 
				                                    and CaseTrsQueryVersion.QDateE <> ''
			                                    THEN CONVERT(NVARCHAR(10), CONVERT(DATETIME, CaseTrsQueryVersion.QDateE), 111) ELSE '' 
		                                    END AS QDateE
		                                    ,ISNULL(BOPS067050Recv.CUSTOMER_NAME, '') AS CUSTOMER_NAME --戶名
		                                    ,ISNULL(CaseCustRFDMRecv.ACCT_NO, CaseTrsQueryVersion.CustAccount) AS ACCT_NO --帳號	X(20)
		                                    ,CASE
			                                    WHEN ISNULL(JRNL_NO,'') = ''
			                                    THEN ''
			                                    ELSE isnull(
		                                    ...
		                                    ,CaseMaster.QueryUserID as TELLER --櫃員代號	X(20)
	                                    ...
	                                    left join
		                                    CaseCustRFDMRecv
	                                    on CaseCustRFDMRecv.VersionNewID = CaseTrsQueryVersion.NewID
	                                    left join
		                                    BOPS067050Recv
	                                    on BOPS067050Recv.VersionNewID = CaseTrsQueryVersion.NewID
	                                        and BOPS067050Recv.CUST_ID_NO = CaseTrsQueryVersion.CustId
	                                    ...
	                                    where 
		                                    CaseTrsQueryVersion.Status <> '99'
                                            and CaseTrsQueryVersion.TransactionFlag = 'Y'
                                            and  CaseTrsQueryVersion.RFDMSendStatus = '99'
		                                    and CaseTrsQueryVersion.CaseTrsNewID = @CaseTrsNewID
                                    ) as temp
                                    ORDER BY 
	                                    GroupId
        ...
        /// <param name="pDocNo"></param>
        /// <returns></returns>
        public DataTable GetRFDMRecvGroupData(string parCaseTrsNewID)
        {
            DataTable dtReturn = new DataTable();
