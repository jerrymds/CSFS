...
namespace CTBC.WinExe.CSFS.NewHistoryFile
{
    public class BBRule
    {
        ...
        public BBRule(string connectstring)
        {
            _strConnectionString = connectstring;
            // 20221201, 改成讀DB 參數檔
            string sql = "Select * from [PARMCode] where CodeType='DBConnectionTimeout';";

            List<PARMCode> ret = SearchList<PARMCode>(sql).ToList();

            if (ret.Count() > 0)
            {
                commandTimeOut = int.Parse(ret.First().CodeNo);
            }
        }
        ...
        protected int ExecuteNonQuery(string sSql, CommandParameterCollection CPC)
        {
            int rtn = 0;
            SqlParameter newOP;
            ...
            {
                using (SqlCommand NewCommand = new SqlCommand(sSql, AConnection))
                {
                    foreach (CommandParameter NewParameter in CPC)
                    {
                        if (sSql.ToUpper().Contains(NewParameter.ColumnName.Replace("@", "").ToUpper()))
                        {
                            newOP = new SqlParameter(NewParameter.ColumnName, NewParameter.Value ?? DBNull.Value);
                            NewCommand.Parameters.Add(newOP);
                        }
                    }
                    NewCommand.CommandTimeout = commandTimeOut;   // 讀取Config檔.. Command TimeOut 的時間...
        ...
        protected int ExecuteNonQuery(string Sql, bool bPrepare, CommandParameterCollection CPC)
        {
            int rtn = 0;
            SqlParameter newOP;
        ...
        public DataTable Search(string Sql, CommandParameterCollection CPC)
        {
