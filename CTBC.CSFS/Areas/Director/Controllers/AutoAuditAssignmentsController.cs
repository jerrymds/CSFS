...
{
    public class AutoAuditAssignmentsController : AppController
    {
        AutoAuditAssignmentsBIZ _directorBiz;
        public AutoAuditAssignmentsController()
        {
            _directorBiz = new AutoAuditAssignmentsBIZ(this);
        }
        [RootPageFilter]
        ...
        public void InitDropdownListOptions()
        {
            ...
            IList<PARMCode> ReceiveKindList =parm.GetCodeData("INCOME_TYPE");
            List<PARMCode> ReceiveKindSel =ReceiveKindList.Where(c => c.CodeDesc == "紙本").ToList();
            foreach (PARMCode item in ReceiveKindSel)
            {
                  ReceiveKindList.Remove(item);
            }
            SelectList ReceiveKindSelList = new SelectList(ReceiveKindList, "CodeDesc", "CodeDesc");
            ViewBag.ReceiveKindList = ReceiveKindSelList;//new SelectList(parm.GetCodeData("INCOME_TYPE"), "CodeDesc", "CodeDesc");  //* 來文方式

            IList<PARMCode> CaseKindList = parm.GetCodeData("CASE_KIND");
            List<PARMCode> CaseKindSel = CaseKindList.Where(c => c.CodeDesc == "外來文案件").ToList();
            foreach (PARMCode item in CaseKindSel)
            {
                 CaseKindList.Remove(item);
            }
            SelectList CaseKindSelList = new SelectList(CaseKindList, "CodeDesc", "CodeDesc");
            ViewBag.CaseKindList = CaseKindSelList;
            IList<PARMCode> CaseKind2List = parm.GetCodeData("CASE_SEIZURE");
            List<PARMCode> CaseKind2Sel = CaseKind2List.Where(c => c.CodeDesc== "扣押並支付").ToList();
            foreach (PARMCode item in CaseKind2Sel)
            {

                 CaseKind2List.Remove(item);
            }
            SelectList CaseKind2SelList = new SelectList(CaseKind2List, "CodeDesc", "CodeDesc");
            ViewBag.CaseKind2List = CaseKind2SelList;
            //Add by zhangwei 20180315 start
        ...
        //Add by zhangwei 20180315 start
        public JsonResult CaseKind1Change(string caseKind)
        {
            PARMCodeBIZ parm = new PARMCodeBIZ();
            ...
            List<PARMCode> CaseKind2Sel = list.Where(c => c.CodeDesc == "扣押並支付").ToList();
            foreach (PARMCode item in CaseKind2Sel)
            {
        ...
        public ActionResult Remit()
        {
            ...
            else
            {
                if (model.CaseKind2 == "扣押" || model.CaseKind2 == "扣押並支付")
                {
                    ...
                    ms = aotuAssignBiz.ExcelForSeizure(headerColumns, model, UserId,"2");
                }
                else//撤銷
                {
                    string[] headerColumns = new[]
                   {
                    ...
                    ms = aotuAssignBiz.ExcelForUndo(headerColumns, model, UserId,"2");
                }
            }

            var fileName = "自動化審核作業" + "_" + Lang.csfs_debtexcel + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
