...
{
    public class DirectorCooperativeController : AppController
    {
        DirectorToApproveBIZ _directorBiz;
        public DirectorCooperativeController()
        {
        ...
        {
            DirectorCooperative model = new DirectorCooperative();
            if (isBack == "1")
            {
        ...
        [HttpPost]
        public ActionResult _QueryResult(DirectorCooperative model, int pageNum = 1, string strSortExpression = "LimitDate", string strSortDirection = "asc")
        {
            #region Cookie
            ...
            IList<DirectorCooperative> list = _directorBiz.GetDirectorCooperativeData(model, pageNum, strSortExpression, strSortDirection, userId);
            var dtvm = new DirectorCooperativeViewModel()
            {
                DirectorCooperative = model,
                DirectorCooperativelist = list,
            };
            //分頁相關設定
            dtvm.DirectorCooperative.PageSize = _directorBiz.PageSize;
            dtvm.DirectorCooperative.CurrentPage = _directorBiz.PageIndex;
            dtvm.DirectorCooperative.TotalItemCount = _directorBiz.DataRecords;
            dtvm.DirectorCooperative.SortExpression = strSortExpression;
            dtvm.DirectorCooperative.SortDirection = strSortDirection;

            dtvm.DirectorCooperative.AssignId = model.AssignId;
            dtvm.DirectorCooperative.CaseNo = model.CaseNo;
            dtvm.DirectorCooperative.GovUnit = model.GovUnit;
            dtvm.DirectorCooperative.GovDate = model.GovDate;
            dtvm.DirectorCooperative.GovNo = model.GovNo;
            dtvm.DirectorCooperative.Person = model.Person;
            dtvm.DirectorCooperative.CaseKind = model.CaseKind;
            dtvm.DirectorCooperative.CaseKind2 = model.CaseKind2;
            dtvm.DirectorCooperative.Speed = model.Speed;
            dtvm.DirectorCooperative.LimitDate = model.LimitDate;
            dtvm.DirectorCooperative.SendKind = model.SendKind;
            dtvm.DirectorCooperative.Status = model.Status;
        ...
        public ActionResult FangXing(string statusArr, string caseIdarr)
        {
            ...
            return Json(new DirectorToApproveBIZ().DirectorApprove(aryStatus, aryCaseId, LogonUser.Account));
        }
        ...
        public ActionResult ChenHe(string statusArr, string caseIdarr)
        {
            ...
            string agentIdList = atoHbiz.GetManagerID(LogonUser.Account);
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Return(DirectorToApprove model, string caseIdList, string statusArr)
        {
            string[] caseid = caseIdList.Split(',');
            string[] status = statusArr.Split(',');
            List<Guid> aryCaseId = (from id in caseid where !string.IsNullOrEmpty(id) select new Guid(id)).ToList();
            List<String> aryStatus = (from s in status where !string.IsNullOrEmpty(s) select s).ToList();
            string userId = LogonUser.Account;
        ...
        public ActionResult Remit()
        {
            ...
            else
            {
                if (model.CaseKind2 == "扣押"|| model.CaseKind2 == "扣押並支付")
                {
                    ...
                    ms = aotuAssignBiz.ExcelForSeizure(headerColumns, model, UserId,"1");
                }
                else//撤銷
                {
                    string[] headerColumns = new[]
                  {
                    ...
                    ms = aotuAssignBiz.ExcelForUndo(headerColumns, model, UserId,"1");
                }
            }
            ...
            var fileName = "協同作業" + "_" + Lang.csfs_debtexcel + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
