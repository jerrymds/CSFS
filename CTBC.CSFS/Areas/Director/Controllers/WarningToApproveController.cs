...
{
    public class WarningToApproveController : AppController
    {
        DirectorToApproveBIZ _directorBiz;
        CustomerInfoBIZ CIBZ = new CustomerInfoBIZ();
        WarningToApproveBIZ wqBiz = new WarningToApproveBIZ();
        // GET: /QueryAndExport/WarningToApprove/
        [RootPageFilter]
        public ActionResult Index()
        {
            WarningToApprove model = new WarningToApprove();
        ...
        public void BindDDl()
        {
            ...
            List<SelectListItem> Kindlist = new List<SelectListItem>
            {
                ...
                new SelectListItem() {Text ="修改聯徵", Value = "修改聯徵"},
        ...
        /// 結果列表
        /// </summary>
        public ActionResult _QueryResult(WarningToApprove model)
        {
        ...
        public ActionResult FangXing(string statusArr, string caseIdarr)
        {
            ...
            return Json(wqBiz.DirectorApprove(aryStatus, aryCaseId, userId));
        }
        [HttpPost]
        public ActionResult Return(WarningToApprove model, string strIds, string statusArr)
        {
            string[] caseid = strIds.Split(',');
            string[] status = statusArr.Split(',');
            List<Guid> aryCaseId = (from id in caseid where !string.IsNullOrEmpty(id) select new Guid(id)).ToList();
            //List<String> aryCaseId = (from id in caseid where !string.IsNullOrEmpty(id) select id).ToList();
            List<String> aryStatus = (from s in status where !string.IsNullOrEmpty(s) select s).ToList();
            string userId = LogonUser.Account;
            //return Json(wqBiz.DirectorReturn(model, aryCaseId, aryStatus[0], userId));
            return Json(wqBiz.DirectorReturn(aryStatus, aryCaseId, userId));
        }
        //[HttpPost]
        //public ActionResult Return(string statusArr, string caseIdarr)
        //{
        //    string userId = LogonUser.Account;
        //    string[] caseid = caseIdarr.Split(',');
        //    string[] status = statusArr.Split(',');
        ...
        //    //List<Guid> aryCaseId = (from id in caseid where !string.IsNullOrEmpty(id) select new Guid(id)).ToList();
        //    //List<String> aryStatus = (from s in status where !string.IsNullOrEmpty(s) select s).ToList();
        //    //return Json(wqBiz.DirectorReturn(aryCaseId, aryStatus[0], userId));
        //}
        /// <summary>
        /// 實際查詢動作
        /// </summary>
        public WarningToApproveViewModel SearchList(WarningToApprove model)
        {
            string CustId = "";
            // 新增個資LOG
            ...
            IList<WarningToApprove> result = wqBiz.GetQueryList(model);
            if (result.Count > 0)
            {
            ...
            var viewModel = new WarningToApproveViewModel()
            {
                WarningToApprove = model,
                WarningToApproveList = result,
            };
        ...
        public ActionResult Excel(WarningToApprove model)
        {
        ...
        private IList<WarningToApprove> GetQueryList(WarningToApprove model)
        {
