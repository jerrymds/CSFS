...
{
    public class AgentCaseCalculatorController : AppController
    {
        ...
        public ActionResult CreateCalculatorDetail(Decimal amount, Decimal interestRate, DateTime startDate, DateTime endDate, Guid caseId, string caseType)
        {
            ...
            if (caseType == "D")        //*日利率
            {
                ...
                decimal interests = Math.Round(amount * interestRate * days / 100, 2, MidpointRounding.AwayFromZero);//* 計算利率
            ...
            if (caseType == "Y")        //*年利率
            {
                ...
                {
                    int days = (endDate - startDate).Days + 1;//* 計算天數  
                    decimal interests = Math.Round(amount * interestRate * days / 365 / 100, 2, MidpointRounding.AwayFromZero);//* 計算利率
                ...
                if (endDate.Year - startDate.Year > 0)     //* 年份不同時，插入多筆數據
                {
                    using (TransactionScope ts = new TransactionScope())
                    {
                        ...
                        decimal interests1 = Math.Round(amount * interestRate * days1 / 365 / 100, 2, MidpointRounding.AwayFromZero);//* 計算第一階段利率
                        ...
                        for (int i = 0; i < (endDate.Year - startDate.Year - 1); i++)
                        {
                            ...
                            daysamount = (Convert.ToDateTime(end) - Convert.ToDateTime(start)).Days + 1;
                            decimal interestsamount = Math.Round(amount * interestRate * daysamount / 365 / 100, 2, MidpointRounding.AwayFromZero);//* 計算多個階段的利率
                        ...
                        decimal interests2 = Math.Round(amount * interestRate * days2 / 365 / 100, 2, MidpointRounding.AwayFromZero);//* 計算最後階段利率
