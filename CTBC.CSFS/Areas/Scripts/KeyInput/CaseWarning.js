...
(function () {
    ...
    jQuery.extend(caseWarning, {
        documentReady: function () {
            ...
            if ($("#NowPage").val() === "EditWarnAccount") {//*編輯帳號內容
                $.CaseWarning.initEditWarnAccount();
            }
        ...
        initCreateWarn: function () {
            ...
            function BtnSettingClick()
            {
                ...
                $.ajax({
                    ...
                    async: false,
                    url: $("#SettingUrl").val(),
                    data: { DocNo: DocNo, NotificationSource: NotificationSource, codeflag: codeflag, Setdate: Setdate, Setdatetime: Setdatetime ,Currency: Currency},
                    dataType: "json",
                    error: function () {
        ...
        initEditWarnAccount: function () {
            $("#btnSaveEditWarnAccount").click(function () { BtnSaveEditWarnAccountClick(); });
            function BtnSaveEditWarnAccountClick() {
                var newLine = "<br/>";
                var msg = "";
                var re = /^([0-9.]+)$/;
                ...
                //* 交易日期
                if ($("#txtTRAN_DATE").val().length <= 0) {
                    msg = msg + $.validator.format($("#PlzInput").val(), $("#NametxtTRAN_DATE").val()) + newLine;
                } else if (!reDate.test($("#txtTRAN_DATE").val())) {
                    msg = msg + $.validator.format($("#PlzCorrectFormat").val(), $("#NametxtTRAN_DATE").val()) + newLine;
                }

                //if ($("#txteTabs").val().length <= 0) {
                //    msg = msg + $.validator.format($("#PlzInput").val(), $("#NametxteTabs").val()) + newLine;
                //} else if (!re.test($("#txteTabs").val())) {
                //    msg = msg + $.validator.format($("#PlzCorrectFormat").val(), $("#NametxteTabs").val()) + newLine;
                //}
                //if ($("#txtMemo").val().length <= 0) {
                //    msg = msg + $.validator.format($("#PlzInput").val(), $("#NametxtMemo").val()) + newLine;
                //}



                //if ($("#txtTimeLog").val().length <= 0) {
                //    msg = msg + $.validator.format($("#PlzInput").val(), $("#NametxtTimeLog").val()) + newLine;
                //}
        ...
        initSetStatus: function () {
            ...
            function btnCancelRemoveClick()
            {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: { DocNo: DocNo, NotificationSource: NotificationSource },
                    error: function () {
                        jAlertError($("#LoadErrorMsg").val());
                        $.unblockUI();
                    },
            ...
            function btnRemoveClick()
            {
                ...
                $.ajax({
                    ...
                    url: $("#RemoveUrl").val(),
                    dataType: "json",
                    data: { DocNo: DocNo, NotificationSource: NotificationSource, EtabsNo: EtabsNo },
                    error: function () {
                        jAlertError($("#LoadErrorMsg").val());
                        $.unblockUI();
                    },
