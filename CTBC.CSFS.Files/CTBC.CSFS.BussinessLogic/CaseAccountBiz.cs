    ...
    public class CaseAccountBiz : CommonBIZ
    {
		...
		public IList<CaseSeizure> GetCaseSeizure(Guid caseId, IDbTransaction trans = null)
		{
			string strSql = @";with  T1 as(SELECT s.[SeizureId],s.[CaseId],s.[PayCaseId],s.[CaseNo],s.[CustId],s.[CustName],s.[BranchNo],s.[BranchName],s.[Account],s.[AccountStatus],s.[Currency],s.[Balance],
                            ...
                            ,Convert(Nvarchar(10), m.GovDate,111) as GovDate,m.GovNo,m.GovUnit,s.[ProdCode],s.[Link],s.[SegmentCode],s.[OtherSeizure],s.[TxtStatus],s.Seq,d.[ProdCode] as TxProdCode,ISNULL(d.SNO,9999999) as sno--,g.ObligorId
                            from CaseSeizure s  
                            ...
                            FROM [TX_60491_Grp]
                            WHERE TX_60491_Grp.CaseID = @CaseId and [CustomerId] IN (SELECT [ObligorNo] FROM [CaseObligor] WHERE [CaseId] = @CaseId)) h on h.CustomerId = s.CustId                             
		...
		//新增操作
		public void Add(Guid CaseId, CaseSeizure formItem, Guid TXSNO, DateTime TXDateTime)
		{
		...
		public IList<CaseSeizure> GetCaseSeizureFromTx(Guid caseId)
		{
            ...
            string strSql = @";with  T1 as(SELECT @CaseId AS [CaseId],M.[CustomerId] AS [CustId],M.[CustomerName] AS [CustName],
							...
							D.[Ccy] AS [Currency],D.[Bal] AS [Balance],D.[ProdDesc] as [ProdCode],D.[Link],D.[System],
							CASE d.[SegmentCode] WHEN '1' THEN '個金' WHEN '2' THEN '法金' WHEN '3' THEN 'SBG' ELSE '' END AS [SegmentCode],
							...
							ROW_NUMBER() OVER (PARTITION BY [CustomerId] ORDER BY [SNO] DESC) AS RowID, [SNO],[CustomerId],[CustomerName]
							FROM [TX_60491_Grp] 
							WHERE TX_60491_Grp.CaseID = @CaseId and [CustomerId] IN (SELECT [ObligorNo] FROM [CaseObligor] WHERE [CaseId] = @CaseId)
							) AS M
			...
			if (list != null && list.Any())
			{
				...
				foreach (CaseSeizure seizure in list)
				{
					...
					{
						CTBC.CSFS.Models.TX_33401 tx33401 = custBiz.GetLatestTx33401(seizure.Account, caseId);//adam 20160427
						if (tx33401 != null)
						{
					...
					if (seizure.Currency.ToUpper() != "TWD" && seizure.Account.Length >= 15)
					{
        ...
        public bool InsertSeizureSetting(CaseSeizure item, IDbTransaction trans = null)
        {
            ...
            string strSql = @"INSERT INTO [CaseSeizure]
                            ...
                            ,[CreatedUser],[CreatedDate],[ProdCode],[Link],[SegmentCode],[OtherSeizure],[TxtStatus],[Seq])VALUES
                            (@CaseId,@CaseNo,@CustId,@CustName,@BranchNo,@BranchName,@Account,@AccountStatus
                            ,@Currency,@Balance,@SeizureAmount,@ExchangeRate,@SeizureAmountNtd,0,@SeizureStatus,@CreatedUser,GETDATE(),@ProdCode,@Link,@SegmentCode,@OtherSeizure,@TxtStatus,@Seq)";
            Parameter.Clear();
        ...
        public IList<CaseSeizure> GetCaseSeizureWithPaySetting(Guid caseId, IDbTransaction trans = null)
        {
            string strSql = ";with  T1 as(SELECT " +
                            ...
                            "s.[SeizureAmount],s.[ExchangeRate],s.[SeizureAmountNtd],s.[PayAmount],s.[TripAmount],s.[SeizureStatus],s.[CreatedUser],s.[CreatedDate],s.[ModifiedUser],s.[ModifiedDate] " +
                             " , Convert(Nvarchar(10),m.GovDate,111) as GovDate,m.GovNo,m.GovUnit " +
                            "FROM [CaseSeizure] s left join CaseMaster m on s.CaseId=m.CaseId " +
                            "WHERE s.[PayCaseId] = @CaseId AND s.[SeizureStatus] = @SeizureStatus) ,T2 as (select ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from CaseDataLog c1,CaseSeizure " +
        ...
        public IList<CaseSeizure> GetCaseSeizureWithoutPaySettingByCustomerList(Guid caseId)
        {
            string strSql = "SELECT " +
                            ...
                            ",Convert(Nvarchar(10), m.GovDate,111) as GovDate,m.GovNo,m.GovUnit from CaseSeizure s left join CaseMaster m on s.CaseId=m.CaseId " +
                            "WHERE s.[CustId] IN (SELECT [ObligorNo] FROM [CaseObligor] WHERE CaseId = @CaseId) AND s.[SeizureStatus] = @SeizureStatus   order by  m.CaseNo,m.GovDate,m.GovUnit,s.Account";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            Parameter.Add(new CommandParameter("@SeizureStatus", SeizureStatus.AfterSeizureSetting));
        ...
        public bool SavePaySetting(List<CaseSeizure> saveList, Guid CaseId)
        {
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    dbTransaction = dbConnection.BeginTransaction();
                    IList<CaseSeizure> listOld = GetCaseSeizureWithPaySetting(CaseId, dbTransaction);
        ...
        /// </summary>
        /// <returns></returns>
        public string SeizureTelegram(CaseSeizure item, string memo, Guid CaseId, string CaseKind2, string BreakDay, string kind)
        {
            bool rtn = false;
            string result = "發查電文后,更新表失敗";
            ExecuteHTG objHTG = new ExecuteHTG();
            CustomerInfoBIZ CIBZ = new CustomerInfoBIZ();
            string retS = objHTG.Send9091Or9093(item.Account, item.SeizureAmount, item.Currency, "", memo, item.CustId, CaseId.ToString());
            if (retS.StartsWith("0000"))
            {
                ...
                if(kind == "Seizure")
                {
                    ...
                    //item.Balance = tx09093.TrueAmt;//可用餘額
                    item.SeizureAmount = item.SeizureAmount;//ETABS扣押金額
                    item.SeizureAmountNtd = item.SeizureAmount * item.ExchangeRate;//台幣
                    string newMemo = "";//備註回文
                    CTBC.CSFS.Models.TX_00450 CMOther = CIBZ.GetLatestTx00450(item.Account, CaseId, "31");
                    if (CMOther != null && CMOther.DATA1.Contains(" 9091 ") && CMOther.DATA2.Contains(" 04 "))
                    {
                        item.OtherSeizure = "Y";//他案扣押
                        newMemo = "帳戶因他案已扣押在案。(請查明)";//他案扣押的備註回文
                    }
                    CTBC.CSFS.Models.TX_00450 CM = CIBZ.GetLatestTx00450(item.Account, CaseId, "30");
                    if (CM != null)
                    {
                        //凍結 備註回文
                        if (CM.DATA2.Contains(" 04 "))
                        {
                            newMemo = "帳戶因他案已凍結在案。(請查明)";
                        }
                        //定存 備註回文
                        if (CM.DATA2.Contains(" 05 ") || CM.DATA2.Contains(" 06 "))
                        {
                            newMemo = "帳戶因他案已設定質權在案。(請查明)";
                        }
                        //其它 備註回文
                        if (CM.DATA2.Contains(" 10 "))
                        {
                            newMemo = "帳戶因其他原因已凍結在案。(請查明)";
                        }
                        //警示 備註回文
                        if (CM.DATA2.Contains(" 11 ") || CM.DATA2.Contains(" 12 "))
                        {
                            newMemo = "帳戶已設定為警示帳戶。";
                        }
                        if (CM.DATA2.Contains(" 13 "))
                        {
                            newMemo = "帳戶已設定為衍生警示帳戶。";
                        }
                        if (CM.DATA2.Contains(" 14 "))
                        {
                            newMemo = "帳戶因偵辦刑事案件設定為警示帳戶。";
                        }
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public bool ResetTelegram(CaseSeizure item, string memo, Guid CaseId, string kind)
        {
            bool rtn = false;
            ExecuteHTG objHTG = new ExecuteHTG();
            string retS = objHTG.Send9092Or9095(item.Account, item.SeizureAmount, item.Currency, "66", memo, item.CustId, CaseId.ToString());
            if (retS.StartsWith("0000"))
            {
                //CaseSeizure csOld = GetCaseSeizureInfo(CaseId, item.SeizureId);
                CaseSeizure csOld = GetCaseSeizureInfoBySeizureId(item.SeizureId);
        ...
        /// </summary>
        /// <returns></returns>
        public bool AccidentTelegram(CaseSeizure item, string kind)
        {
            bool rtn = false;
            ExecuteHTG objHTG = new ExecuteHTG();
            CustomerInfoBIZ CIBZ = new CustomerInfoBIZ();
            bool? retA = objHTG.is405Accident(item.CustId,item.CaseId.ToString(),item.Account);
        ...
        /// </summary>
        /// <returns></returns>
        public bool PayTelegram(CaseSeizure item, string memo, Guid CaseId, string BreakDay, string kind)
        {
            bool rtn = false;
            ExecuteHTG objHTG = new ExecuteHTG();
            string retP = "";
            ...
            if (kind == "Pay")//支付
            {
                ...
                {
                    retP = objHTG.Send9091Or9093(item.Account, item.SeizureAmount, item.Currency, "", memo, item.CustId, CaseId.ToString(), "1", BreakDayToTelegram);
                }
                else//支付金額≠扣押金額
                {
                    retP = objHTG.Send9092Or9095(item.Account, item.SeizureAmount, item.Currency, "", memo, item.CustId, CaseId.ToString(), "0", BreakDayToTelegram);
                    retP = objHTG.Send9091Or9093(item.Account, item.PayAmount, item.Currency, "", memo, item.CustId, CaseId.ToString(), "0", BreakDayToTelegram);
                }
            }
            else if (kind == "Reset")//沖正
            {
                retP = objHTG.Send9091Or9093(item.Account, item.PayAmount, item.Currency, "", memo, item.CustId, CaseId.ToString(), "0", "");
            }
