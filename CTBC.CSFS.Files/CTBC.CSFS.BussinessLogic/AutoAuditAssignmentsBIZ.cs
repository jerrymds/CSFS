...
namespace CTBC.CSFS.BussinessLogic
{
    public class AutoAuditAssignmentsBIZ : CommonBIZ
    {
            ...
            if (ilst != null)
            {
                ms = ExcelExport(ilst, headerColumns,
                                                   delegate (HSSFRow dataRow, RemitSeizure dataItem)
                                                   {
                                                       ...
                                                       dataRow.CreateCell(2).SetCellValue(dataItem.ObligorName);
                                                       dataRow.CreateCell(3).SetCellValue(dataItem.Agent);
                                                       dataRow.CreateCell(4).SetCellValue(dataItem.RegistrationNo);
                                                       dataRow.CreateCell(5).SetCellValue(dataItem.ObligorNo);
                                                       dataRow.CreateCell(6).SetCellValue(dataItem.ReceiveAmount);
                                                       dataRow.CreateCell(7).SetCellValue(dataItem.Account);
                                                       ...
                                                       dataRow.CreateCell(13).SetCellValue(dataItem.Memo);
                                                   });
            }
            return ms;
        }
            ...
            if (ilst != null)
            {
                ms = ExcelExport(ilst, headerColumns,
                                                   delegate (HSSFRow dataRow, RemitSeizure dataItem)
                                                   {
                                                       ...
                                                       dataRow.CreateCell(3).SetCellValue(dataItem.Currency);
                                                       dataRow.CreateCell(4).SetCellValue(dataItem.SendNo);
                                                       dataRow.CreateCell(5).SetCellValue(dataItem.SeizureAmount);
                                                       dataRow.CreateCell(6).SetCellValue(dataItem.CancelAmount);
        ...
        public IList<RemitSeizure> GetPrintDataForSeizure(DirectorToApprove model, string EmpId, string strManageType)
        {
            try
            {
                ...
                string strSql = @"select* from(  (select NT.*,F.Memo from ( SELECT distinct B.CaseKind2,B.CaseId,B.CaseNo,E.ObligorName,E.ObligorName as Agent,E.ObligorNo as RegistrationNo,E.ObligorNo ,B.ReceiveAmount,D.Account,D.Currency,D.SeizureAmount,D.Balance-D.SeizureAmount as AvailBalance,B.AgentUser,B.CreatedDate,B.ReceiveKind FROM CaseAssignTable AS A
    LEFT  JOIN CaseMaster AS B ON A.CaseId=B.CaseId 
    left join CaseSeizure D on D.CaseId=B.CaseId left join CaseObligor E on B.CaseId=E.CaseId 
    WHERE B.ReceiveKind='紙本'  
    AND A.AlreadyAssign=0 
    AND (B.Status=@Status1 OR B.Status=@Status2) AND B.isDelete='0' " + sqlWhere + @" 
    ) NT left join  CaseMemo F on F.CaseId=NT.CaseId)
    union all
    ( select NT.*,F.Memo from (SELECT distinct B.CaseKind2,B.CaseId,B.CaseNo,E.ObligorName,E.Agent,E.RegistrationNo,E.ObligorNo,E.Total as ReceiveAmount,D.Account,D.Currency,D.SeizureAmount,D.Balance-D.SeizureAmount as AvailBalance,B.AgentUser,B.CreatedDate,B.ReceiveKind FROM CaseAssignTable AS A
    LEFT  JOIN CaseMaster AS B ON A.CaseId=B.CaseId 
    left join CaseSeizure D on D.CaseId=B.CaseId left join EDocTXT1 E on B.CaseId=E.CaseId 
    WHERE B.ReceiveKind='電子公文'  
    AND A.AlreadyAssign=0 
    AND (B.Status=@Status1 OR B.Status=@Status2) AND B.isDelete='0' " + sqlWhere + @" 
    ) NT left join  CaseMemo F on F.CaseId=NT.CaseId) ) SeizureTable where 1=1 " + sqlwhere1 + @" 
    order by CaseNo ASC";
        ...
        public IList<RemitSeizure> GetPrintDataForUndo(DirectorToApprove model, string EmpId, string strManageType)
        {
            try
            {
                ...
                string strSql = @"select* from(  (select NT.*,F.Memo from ( SELECT distinct B.CaseKind2,B.CaseId,B.CaseNo,D.Account,D.Currency,E.ObligorNo as SendNo,D.SeizureAmount,D.CancelAmount,B.AgentUser,B.CreatedDate,B.ReceiveKind FROM CaseAssignTable AS A
    LEFT  JOIN CaseMaster AS B ON A.CaseId=B.CaseId 
    left join CaseSeizure D on D.CaseId=B.CaseId left join CaseObligor E on B.CaseId=E.CaseId 
    WHERE B.ReceiveKind='紙本' 
    AND A.AlreadyAssign=0 
    ...
    union all
    ( select NT.*,F.Memo from (SELECT distinct B.CaseKind2,B.CaseId,B.CaseNo,D.Account,D.Currency,E.ReceiverNo as SendNo,D.SeizureAmount,E.Amount as CancelAmount,B.AgentUser,B.CreatedDate,B.ReceiveKind FROM CaseAssignTable AS A
    LEFT  JOIN CaseMaster AS B ON A.CaseId=B.CaseId 
    left join CaseSeizure D on D.CaseId=B.CaseId left join EDocTXT2 E on B.CaseId=E.CaseId 
    WHERE B.ReceiveKind='電子公文' 
    AND A.AlreadyAssign=0 
    AND (B.Status=@Status1 OR B.Status=@Status2) AND B.isDelete='0' " + sqlWhere + @" 
    ) NT left join  CaseMemo F on F.CaseId=NT.CaseId) ) SeizureTable where 1=1 " + sqlwhere1 + @" 
