    ...
    class Program : BaseBusinessRule
    {
        ...
        private void Process()
        {
           ...
           // ExcelZip();
            ...
            // HTG發查產生成功 並且 RFDF回文產生成功，將Version狀態更新成成功或者重查成功
            ModifyVersionStatus1();

            // HTG發查產生成功 並且 RFDF回文產生成功，將Version狀態更新成成功或者重查成功
            //ModifyVersionStatus2();
        ...
        {
            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------讀取電文結果（PDF 存款帳戶資料）開始------");
            ...
            if (dtHTGResult != null && dtHTGResult.Rows.Count > 0)
            {
                ...
                for (int p = 0; p < dtHTGResult.Rows.Count; p++)
                {
                    ...
                    if (dt401Recv != null && dt401Recv.Rows.Count > 0)
                    {
                        ...
                        {
                            //txtDocIDPath = txtDocDirPath + "\\" + dt401Recv.Rows[q]["CUST_ID_NO"].ToString();
                            ...
                            // 記錄LOG
                            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------向" + txtDocIDPath + "文件中增加基本資料!");
                            dt.Clear();
                            DataCount1++;
                        }
                    ...
                    else
                    {
                        // 查無資料
                        //DataRow strRow = dt.NewRow();
                        //strRow["CUST_ID_NO"] = dtHTGResult.Rows[p]["CustId"].ToString()+ dtHTGResult.Rows[p]["CustAccount"].ToString() + "與本行無存款往來";
                        //dt.Rows.Add(strRow);
                        //CreateExcel(dtHTGResult.Rows[p]["CustId"].ToString() + dtHTGResult.Rows[p]["CustAccount"].ToString(), dt, txtDocIDPath);
                        // 記錄LOG
                        m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, dtHTGResult.Rows[p]["CustId"].ToString() + dtHTGResult.Rows[p]["CustAccount"].ToString() + "與本行無存款往來");
                    }

                    // 更新HTGSendStatus狀態
                    if (dt401Recv != null && dt401Recv.Rows.Count > 0)
                    {
                        int HTGCount = UpdatePDFHTGSendStatus(dtHTGResult.Rows[p]["CaseTrsNewID"].ToString());

                        // 記錄LOG
                        if (HTGCount > 0)
                        {
                            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------更新CaseTrsQueryVersion表CaseTrsNewID=" + dtHTGResult.Rows[p]["CaseTrsNewID"].ToString() + "的HTGSendStatus='99'");
                        }
                    }
                }
            }
            ...
            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------匯出檔（存款帳戶開戶資料）結束------");
        }
        ...
        public void PdfZip()
        {
            ...
            if (dtHTGzipResult != null && dtHTGzipResult.Rows.Count > 0)
            {
                for (int p = 0; p < dtHTGzipResult.Rows.Count; p++)
                {
                    ...
                    // 上傳到CaseEdocFile
                    Guid caseId = (Guid)dtHTGzipResult.Rows[0]["CaseTrsNewID"];
                    ...
                    caseEdocFile.FileName = Path.GetFileName(pdfDocDirPath) + ".pdfzip";
        ...
        public void ExcelZip()
        {
            ...
            if (dtHTGzipResult != null && dtHTGzipResult.Rows.Count > 0)
            {
                for (int p = 0; p < dtHTGzipResult.Rows.Count; p++)
                {
                    ...
                    // 上傳到CaseEdocFile
                    Guid caseId = (Guid)dtHTGzipResult.Rows[p]["CaseTrsNewID"];
                    ...
                    caseEdocFile.FileName = Path.GetFileName(txtDocDirPath) + ".xlszip";
                    caseEdocFile.FileObject = ZipBytes;
                    caseEdocFile.SendNo = "";
                    InsertCaseEdocFile(caseEdocFile);
        ...
        {
            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------讀取電文結果（存款帳戶開戶資料）開始------");
            ...
            if (dtHTGResult != null && dtHTGResult.Rows.Count > 0)
            {
                ...
                for (int p = 0; p < dtHTGResult.Rows.Count; p++)
                {
                    ...
                    //if (dt401Recv != null && dt401Recv.Rows.Count > 0)
                    //{
                    //    int HTGCount = UpdateHTGSendStatus(dtHTGResult.Rows[p]["CaseTrsNewID"].ToString());
        ...
        //                    // 換行
        //                    //fileContent1 += "\r\n";
        //                    AppendContent(txtDocIDPath, fileContent1, DataCount1);

        //                    // 記錄LOG
        //                    m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------向" + txtDocIDPath + "文件中增加" + DataCount1 + "筆資料!");
        ...
        public DataTable GetHTGzip()
        {
            string sqlSelect = @"
                               select
                                 distinct CaseTrsNewID,CaseMaster.CaseNo as DocNo
                                from
                                  CaseMaster
                                left join
                                  CaseTrsQueryVersion
                                ...
                                where
                                  OpenFlag = 'Y'
                                  and CaseTrsQueryVersion.HTGSendStatus = '4'
                                  and CaseMaster.Status is not Null  -- in ('02','06')    
                                  and CaseTrsQueryVersion.CaseTrsNewID  in (
                                    ...
                                    on CaseMaster.CaseID = CaseTrsQueryVersion.CaseTrsNewID
                                    where
                                      CaseMaster.Status is not Null  -- in ('02','06')    
                                      and OpenFlag = 'Y'
                                      and CaseTrsQueryVersion.HTGSendStatus  in ('4','88','99')  
                                    group by
                                      CaseMaster.CaseID
                                  );
                              ";
        ...
        public DataTable GetHTGData()
        {
            string sqlSelect = @"
                                ...
                                where
                                  OpenFlag = 'Y'
                                  and CaseTrsQueryVersion.HTGSendStatus = '4'
                                  and CaseTrsQueryVersion.Status <> '03' 
                                  and CaseMaster.Status is not Null  -- in ('02','06')    
                                  and CaseTrsQueryVersion.CaseTrsNewID  in (
                                    select 
                                      CaseMaster.CaseID
...
//                                	,BOPS000401Recv.ACCT_NO --帳號
//									,TX_67100.IdNo02 -- 居留證號
//									,TX_67102.CUST_ID_NO as MASTER_ID --負責人ID
//									,TX_67102.CUST_NAME  as MASTER_NAME --負責人姓名									 
//                                FROM BOPS000401Recv
//								LEFT join TX_67100 
//								    on BOPS000401Recv.VersionNewID = TX_67100.CaseId and CUST_ID_NO=TX_67100.CustId
//								LEFT join TX_67102 
//								    on BOPS000401Recv.VersionNewID = TX_67102.CaseId 
...
//                                order by CaseTrsQueryVersion.CustId,BOPS000401Recv.CUST_ID_NO,OPEN_DATE ";
  ...
  string sqlSelect = @"SELECT 
									...
									,BOPS067050V6Recv.CUST_ID_NO as MASTER_ID --負責人ID
									,BOPS067050V6Recv.CUST_NAME  as MASTER_NAME --負責人姓名									 
                                FROM BOPS067050Recv
								...
								LEFT join BOPS067050V4Recv 
								    on BOPS000401Recv.VersionNewID = BOPS067050V4Recv.VersionNewID and BOPS000401Recv.CUST_ID_NO=BOPS067050V4Recv.CUST_ID_NO
								LEFT join BOPS067050V6Recv  
								    on BOPS000401Recv.VersionNewID = BOPS067050V6Recv.VersionNewID
                                LEFT JOIN CaseTrsQueryVersion
                                 ON CaseTrsQueryVersion.NewId = BOPS067050Recv.VersionNewID
                                WHERE BOPS067050Recv.VersionNewID = @VersionNewID
                                order by CaseTrsQueryVersion.CustId,BOPS067050Recv.CUST_ID_NO,OPEN_DATE";
            // 清空容器
            base.Parameter.Clear();
        ...
        {
            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------產生（存款往來明細資料）開始------");
            // 查詢產生檔案的資料
            ...
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtResult.Rows.Count; i++)
                {
                    ...
                    {  
                        if (!Directory.Exists(pdfFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString())) Directory.CreateDirectory(pdfFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString());
                        txtDocDirPath = pdfFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString();
                        DocNo = dtResult.Rows[i]["DocNo"].ToString();
                    }

                    // 根據主檔主鍵獲取該案件下身分證統一編號資料
                    DataTable drRecvAccount = GetRFDMRecvAccount(dtResult.Rows[i]["CaseTrsNewID"].ToString());
                    ...
                    if (drRecvData != null && drRecvData.Rows.Count > 0)
                    {
                        for (int a = 0; a < drRecvAccount.Rows.Count; a++)
                        {
                            ...
                            Acct_DateFm_DateTo = drRecvData.Rows[a]["ACCT_NO"].ToString() + "-" + drRecvData.Rows[a]["QDateS"].ToString() + "-" + drRecvData.Rows[a]["QDateE"].ToString();
                            // 遍歷並追加txt文件
                            for (int j = 0; j < drRecvData.Rows.Count; j++)
                            {
                                txtDocIDPath = txtDocDirPath + "\\" + drRecvData.Rows[j]["CUST_ID_NO"].ToString();
                                if (!Directory.Exists(txtDocIDPath)) Directory.CreateDirectory(txtDocIDPath);
                                #region 拼接內容
                                DataRow strRow = dt.NewRow();
                                // 身分證統一編號 X(10)
                                strRow["CUST_ID_NO"] = ChangeValue(drRecvData.Rows[j]["CUST_ID_NO"].ToString(), 10);
                                strRow["ACCT_NO"] = ChangeAccount(drRecvData.Rows[j]["ACCT_NO"].ToString(), 20);
                                // 交易序號    9(08)
                                strRow["JRNL_NO"] = ChangeValue(drRecvData.Rows[j]["JRNL_NO"].ToString(), 8);
                                // 交易日期    X(08)
                                strRow["TRAN_DATE"] = ChangeValue(drRecvData.Rows[j]["TRAN_DATE"].ToString(), 8);
                                // 交易時間    X(06)???
                                strRow["JNRST_TIME"] = ChangeValue(drRecvData.Rows[j]["JNRST_TIME"].ToString(), 6);
                                // 交易行(或所屬分行代號)    X(07)
                                strRow["TRAN_BRANCH"] = ChangeValue(drRecvData.Rows[j]["TRAN_BRANCH"].ToString(), 7);
                                // 交易櫃員    X(05)
                                strRow["TRANS_CODE"] = ChangeValue(drRecvData.Rows[j]["TRANS_CODE"].ToString(), 5);
                                // 交易摘要    X(40)
                                strRow["TXN_DESC"] = ChangeChiness(drRecvData.Rows[j]["TXN_DESC"].ToString(), 40);
                                // 支出金額    X(16)
                                ...
                                strRow["TRAN_AMT"] = ChangeValue(drRecvData.Rows[j]["TRAN_AMT"].ToString(), 16);
                                // 存入金額    X(16)
                                // 20180119,PeterHsieh : CTBC修改規格為，當金額為負數就放在支出金額，為正就放在存入金額，另一金額欄位就放'+0'
                                string SaveAMT = drRecvData.Rows[j]["TRAN_AMT"].ToString();
                                SaveAMT = SaveAMT.Contains("-") ? "+0.00" : ("+" + SaveAMT);
                                strRow["SaveAMT"] = ChangeValue(drRecvData.Rows[j]["SaveAMT"].ToString(), 16);
                               // strRow["SaveAMT"] = ChangeNumber(SaveAMT, 16, 2, false);
                                ...
                                //strRow["BALANCE"] = ChangeNumber(strBALANCE, 16, 2, false);
                                strRow["BALANCE"] = ChangeValue(drRecvData.Rows[j]["BALANCE"].ToString(), 16);
                                // 幣別  X(03)
                                string Currency = GetCurrency(drRecvData.Rows[j]["ACCT_NO"].ToString());
                                strRow["Currency"] = Currency;
                                // ATM或端末機代號   X(20)
                                strRow["ATM_NO"] = ChangeValue(drRecvData.Rows[j]["ATM_NO"].ToString(), 20);
                                // 櫃員代號    X(20)
                                strRow["TELLER"] = ChangeValue(drRecvData.Rows[j]["TELLER"].ToString(), 20);
                                // 轉出入行庫代碼及帳號 (RFDM) TRF_BANK + TRF_ACCT  X(20)
                                strRow["TRF_BANK"] = ChangeValue(drRecvData.Rows[j]["TRF_BANK"].ToString(), 20);
                                // 票號(RFDM)  
                                strRow["CHQ_NO"] = ChangeChiness(drRecvData.Rows[j]["CHQ_NO"].ToString(), 8);
                                // 備註(RFDM) NARRATIVE 
                                strRow["NARRATIVE"] = ChangeChiness(drRecvData.Rows[j]["NARRATIVE"].ToString(), 40);
                                // 註記(RFDM) REMARK X(300)
                                strRow["REMARK"] = ChangeChiness(drRecvData.Rows[j]["REMARK"].ToString(), 40);
                                //81019
                                strRow["YBTXLOG_SRC_ID"] = ChangeChiness(drRecvData.Rows[j]["YBTXLOG_SRC_ID"].ToString(), 7); // 設備代理行
                                strRow["YBTXLOG_STAND_NO"] = ChangeChiness(drRecvData.Rows[j]["YBTXLOG_STAND_NO"].ToString(), 7); // 交易序號
                                strRow["YBTXLOG_SAFE_TMNL_ID"] = ChangeChiness(drRecvData.Rows[j]["YBTXLOG_SAFE_TMNL_ID"].ToString(), 8); //機器編號
                                strRow["YBTXLOG_IC_MEMO_CARDNO"] = ChangeChiness(drRecvData.Rows[j]["YBTXLOG_IC_MEMO_CARDNO"].ToString(), 16);//卡號
                                strRow["ADD_NARRATIVE"] = ChangeChiness(drRecvData.Rows[j]["ADD_NARRATIVE"].ToString(), 40); //附言
                                strRow["CUSTOMER_NAME"] = ChangeChiness(drRecvData.Rows[j]["CUSTOMER_NAME"].ToString(), 60); //客戶名稱
                                strRow["PrintDate"] = ChangeChiness(System.DateTime.Now.ToShortDateString(), 10); //列印日期
                                strRow["QDateS"] = ChangeChiness(drRecvData.Rows[j]["QDateS"].ToString(), 10); //查詢起
                                strRow["QDateE"] = ChangeChiness(drRecvData.Rows[j]["QDateE"].ToString(), 10); //查詢迄
                                dt.Rows.Add(strRow);
                                #endregion
                                // 換行
                                //fileContent2 += "\r\n";
                            ...
                            // 記錄LOG
                            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------向" + Acct_DateFm_DateTo + "文件中增加" + drRecvData.Rows.Count + "筆資料!");
                        }
                    }
                    else
                    {
                        ...
                        // 記錄LOG
                        m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------向" + Acct_DateFm_DateTo + "無交易明細資料");
                    }
                ...
                //string zippath = txtDocDirPath;//要壓縮檔案的目錄路徑
                //CreateZip(txtDocDirPath, txtDocDirPath);
                //// 上傳到CaseEdocFile


                //Guid caseId = (Guid)dtResult.Rows[0]["CaseTrsNewID"];
                //string UploadFile = txtDocDirPath + ".zip";
                ...
                //caseEdocFile.Type = "歷史";
                //caseEdocFile.FileType = "pdf";
                //caseEdocFile.FileName = Path.GetFileName(txtDocDirPath) + ".pdfzip";
                //caseEdocFile.FileObject = ZipBytes;
                //caseEdocFile.SendNo = "";
                //InsertCaseEdocFile(caseEdocFile);
            ...
            //    Guid caseId = (Guid)dtResult.Rows[i]["CaseTrsNewID"].ToString();

            //}

            m_fileLog.Write(FileLog.WorkType.Work, FileLog.ErrorType.None, "信息--------交易檔名稱（存款往來明細資料）結束------");
        }
            ...
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtResult.Rows.Count; i++)
                {
                    if (DocNo != dtResult.Rows[i]["DocNo"].ToString())
                    {
                        ...
                        if (!Directory.Exists(txtFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString())) Directory.CreateDirectory(txtFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString());
                        txtDocDirPath = txtFilePath + "\\" + dtResult.Rows[i]["DocNo"].ToString();
                        //string Filename = txtFilePath + "\\" + dtHTGResult.Rows[p]["ROpenFileName"].ToString();
                        // //查詢 CaseEdocFile 是否已存在,存在清空,暫時不處理
                        //if (File.Exists(Filename))
                        // {
                ...
                /*
                 /// adam 原本是交易後,包含基本資料及交易壓縮,但是萬一沒交易,基本資料會沒有壓縮
                string zippath = txtDocDirPath;//要壓縮檔案的目錄路徑
                CreateZip(txtDocDirPath, txtDocDirPath);
                // 上傳到CaseEdocFile
                Guid caseId = (Guid)dtResult.Rows[0]["CaseTrsNewID"];
...
SELECT distinct
                                	...
                                	,CASE WHEN TRAN_AMT < 0 THEN FORMAT(TRAN_AMT*(-1), '#,#.00') ELSE FORMAT(0, '#,#.00') END TRAN_AMT --支出金額	X(16)
                                	,CASE WHEN TRAN_AMT >= 0 THEN FORMAT(TRAN_AMT, '#,#.00') ELSE FORMAT(0, '#,#.00') END as  SaveAMT  --存入金額	X(16)
                                	,FORMAT(BALANCE, '#,#.00') as  BALANCE --餘額	X(16)
                                	,( select top 1 CaseCustATMRecv.[YBTXLOG_SAFE_TMNL_ID]
                                       from CaseCustATMRecv
                                       where CaseCustATMRecv.[YBTXLOG_YYYYMMDD] = CONVERT(Varchar, CaseTrsRFDMRecv.TRAN_DATE, 112)
                                       and ('0'+CaseCustATMRecv.[YBTXLOG_STAND_NO]) = CaseTrsRFDMRecv.FISC_SEQNO
                                       order by CaseCustATMRecv.CreatedDate DESC
                                    ) as ATM_NO --ATM或端末機代號	X(20)
                                	,CaseTrsQueryVersion.CreatedUser as TELLER --櫃員代號	X(20)
                                    ,CHQ_NO   -- 票號
                                    ,REMARK  -- 註記
									,CaseCustATMRecv.YBTXLOG_SRC_ID -- 設備代理行
                                    ,CaseCustATMRecv.YBTXLOG_STAND_NO--  交易序號
                                    ,CaseCustATMRecv.YBTXLOG_SAFE_TMNL_ID --機器編號
                                    ,CaseCustATMRecv.YBTXLOG_IC_MEMO_CARDNO --卡號
                                ...
                                LEFT JOIN CaseTrsQueryVersion ON CaseTrsQueryVersion.NewID = CaseTrsRFDMRecv.VersionNewID
                                LEFT JOIN CaseMaster ON CaseMaster.CaseID = CaseTrsQueryVersion.CaseTrsNewID
								LEFT JOIN CaseCustATMRecv ON CaseCustATMRecv.NewID = CaseTrsQueryVersion.NewID
...
SELECT NEWID
 FROM
 (
	SELECT NEWID from CaseTrsQueryVersion WHERE OpenFlag = 'Y' AND HTGSendStatus = '99' AND (TransactionFlag IS NULL OR TransactionFlag <> 'Y') AND Status = '02'
	UNION
	SELECT NEWID from CaseTrsQueryVersion WHERE TransactionFlag = 'Y' AND RFDMSendStatus = '99' AND (OpenFlag IS NULL OR OpenFlag <> 'Y') AND Status = '02'
	UNION
	SELECT NEWID from CaseTrsQueryVersion WHERE OpenFlag = 'Y' AND HTGSendStatus = '99' AND TransactionFlag = 'Y' AND RFDMSendStatus = '99' AND Status = '02'
)RESULT
WHERE CaseTrsQueryVersion.NewID = RESULT.NewID
); ";
                // 將重查拋查中的案件狀態更新為【重查成功】
//                sql += @"
//UPDATE CaseTrsQueryVersion SET Status = '07' WHERE  EXISTS
//(
//SELECT NEWID
// FROM
// (
//	SELECT NEWID from CaseTrsQueryVersion WHERE OpenFlag = 'Y' AND HTGSendStatus = '99' AND (TransactionFlag IS NULL OR TransactionFlag <> 'Y') AND Status = '06'
//	UNION
//	SELECT NEWID from CaseTrsQueryVersion WHERE TransactionFlag = 'Y' AND RFDMSendStatus = '99' AND (OpenFlag IS NULL OR OpenFlag <> 'Y') AND Status = '06'
//	UNION
//	SELECT NEWID from CaseTrsQueryVersion WHERE OpenFlag = 'Y' AND HTGSendStatus = '99' AND TransactionFlag = 'Y' AND RFDMSendStatus = '99' AND Status = '06'
//)RESULT
//WHERE CaseTrsQueryVersion.NewID = RESULT.NewID
//)
//";
	...
	GROUP BY CaseTrsQueryVersion.CaseTrsNewID, CaseMaster.Status
        ...
        protected void CreateRFDMExcel(string AccountName, DataTable dt, string strFileName)
        {
            ...
            try
            {
                ...
                sheet.SetColumnWidth(13, (int)((10 + 0.72) * 256));//N
                sheet.SetColumnWidth(14, (int)((10 + 0.72) * 256));//O
                sheet.SetColumnWidth(15, (int)((10 + 0.72) * 256));//P
                sheet.SetColumnWidth(16, (int)((10 + 0.72) * 256));//Q
                sheet.SetColumnWidth(17, (int)((30 + 0.72) * 256));//R
                sheet.SetColumnWidth(18, (int)((24 + 0.72) * 256));//S
