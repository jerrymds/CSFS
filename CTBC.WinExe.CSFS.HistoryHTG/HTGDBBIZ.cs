...
namespace CTBC.WinExe.CSFS.HistoryHTG
{
   public class HTGDBBIZ : BaseBusinessRule
   {
      ...
      /// <param name="applNoB"></param>
      /// <returns></returns>
      public bool SendMsgCase(CTCB.NUMS.Library.HTG.HTGObjectMsg obj, string txid, DataRow drApprMsgDefine, string VersionNewID, DataTable CurrencyList,string chkCustAccount ,string chkCustId,string chkCurrency,string chkQDateS,string chkQDateE)
      {
         bool bRet = true;
         ...
         try
         {
            ...
            if (txid == "067050V4")
            {
                DataTable dt670504Data = OpenDataTable("select * from BOPS067050V4Send where SendStatus = '02' and VersionNewID = '" + VersionNewID + "'");
                if (dt670504Data.Rows.Count <= 0)
                {
                    WriteLog("VersionNewID = [" + VersionNewID + "], TxID=[" + txid + "],無發查資料！");
                    return false;
                }
            ...
            if (txid == "067050V6")
            {
                DataTable dt670506Data = OpenDataTable("select * from BOPS067050V6Send where SendStatus = '02' and VersionNewID = '" + VersionNewID + "'");
                if (dt670506Data.Rows.Count <= 0)
                {
                    WriteLog("VersionNewID = [" + VersionNewID + "], TxID=[" + txid + "],無發查資料！");
                    return false;
                }
            ...
            if (txid == "081019")
            {
                DataTable dt081019Data = OpenDataTable("select * from BOPS081019Send where SendStatus = '02' and VersionNewID = '" + VersionNewID + "'");
                if (dt081019Data.Rows.Count <= 0)
                {
                    WriteLog("VersionNewID = [" + VersionNewID + "], TxID=[" + txid + "],無發查資料！");
                    return false;
                }
                else  // 如果發查日期起是空白 ,就去找000401Recv OpenDate 更新 CaseTrsQueryVersion//
                {
                    // adam  考慮一下要不要補
                }
            }
            ...
            for (int n = 0; n < dtData.Rows.Count; n++)
            {
               ...
               if (!result)
               {
                  ...
                  //else
                  {
                     ...
                     // 將CaseTrsQueryVersion表狀態更新為異常
                     EditCaseTrsQueryVersion(VersionNewID, "6", strMsg);
                     bRet = false;
               ...
               else
               {
                  ...
                  if (htgdata.Tables[0].Rows.Count > 0)
                  {
                     try
                     {
                        ...
                        // 067050 發查成功 且 沒有輸入帳號增加60490，將
                        if  (txid == "067050"  && chkCustAccount.Trim().Length < 11)
                        {
                        ...
                        DataTable dOpen_Date = new DataTable();                         
                        // 如果是輸入帳號沒有客戶ID ,000401 發查成功，將新增67050 基本資料
                        if ((txid == "000401"  && chkQDateS.Length < 8) || (txid == "000401"  && chkCustId.Length < 8) )
                        {
                            // 如果  dtData.Rows[n]["QdateS"].ToString().length < 8 先讀取000401 Recv OPEN_DATE 才能打 081019 取
                            string sQDateS = "";
                            string sCustID = "";
                            if (chkQDateS.Length < 8)
                            {
                                dOpen_Date = OpenDataTable("Select OPEN_DATE From BOPS000401Recv where VersionNewID = '" + VersionNewID + "' ");
                                // get000401
                                if (dOpen_Date.Rows[0]["OPEN_DATE"].ToString().Length > 0)
                                {
                                    sQDateS = dOpen_Date.Rows[0]["OPEN_DATE"].ToString().Substring(6, 4) + dOpen_Date.Rows[0]["OPEN_DATE"].ToString().Substring(3, 2) + dOpen_Date.Rows[0]["OPEN_DATE"].ToString().Substring(0, 2);
                                    sCustID = dOpen_Date.Rows[0]["CUST_ID_NO"].ToString();
                                    // 更新 CaseTrsQueryVersion 
                                    UpdateCaseTrsQueryVersionOpenDate(VersionNewID, sQDateS,sCustID);
                                }
                            }
                        }
                        if (txid == "000401"  && chkCustAccount.Trim().Length > 11 &&  chkCustId.Length < 8)
                        {
                            System.Text.RegularExpressions.Regex re = new System.Text.RegularExpressions.Regex("[A-Z|a-z]$");
                            string eng = "";
                            if (dt.Rows[0]["CUST_ID_NO"].ToString() != null && dt.Rows[0]["CUST_ID_NO"].ToString().Length > 7)
                            {
                                eng = dt.Rows[0]["CUST_ID_NO"].ToString().Substring(dt.Rows[0]["CUST_ID_NO"].ToString().Length - 3, 3);
                                if (re.IsMatch(eng))
                                {
                                    Insert67050V4Send(VersionNewID, dt, txid, dtData.Rows[n]["NewID"].ToString());
                                }
                            }
                            if (dt.Rows[0]["CUST_ID_NO"].ToString() != null && dt.Rows[0]["CUST_ID_NO"].ToString().Length == 8)
                            {
                                Insert67050V6Send(VersionNewID, dt, txid, dtData.Rows[n]["NewID"].ToString());
                            }
                            Insert67050Send(VersionNewID, dt, txid, dtData.Rows[n]["NewID"].ToString());
                            Insert081019Send(VersionNewID, dt, txid, dtData.Rows[n]["NewID"].ToString(), _racfbranch, _racfuid.Substring(1, _racfuid.Length - 1), chkCustAccount, chkCurrency);
                        }
                        if (txid == "000401" && chkCustAccount.Trim().Length > 11 && chkCustId.Length > 7)
                        {
                            System.Text.RegularExpressions.Regex re = new System.Text.RegularExpressions.Regex("[A-Z|a-z]$");
                            string eng = "";
                            if (dt.Rows[0]["CUST_ID_NO"].ToString() != null && dt.Rows[0]["CUST_ID_NO"].ToString().Length > 7)
                        ...
                        if (strReturn != "ERROR")
                        {
                           ...
                           if (txid == "067050")
                            {
                                if ( dt.Rows[0]["outputcode"].ToString() == "03" || dt.Rows[0]["outputcode"].ToString() == "08"  || dt.Rows[0]["outputcode"].ToString() == "30"    || dt.Rows[0]["outputcode"].ToString() == "80")
                                {
                                    drSend["QueryErrMsg"] = "";
                                }
                                else
      ...
      /// <returns></returns>
      public bool EditCaseTrsQueryVersion(string strVersionNewID, string strHTGSendStatus, string strHTGQryMessage)
      {
      ...
      public bool UpdateCaseTrsQueryVersionOpenDate(string strVersionNewID, string QDateS,string CustId)
      {
          string sql = @"UPDATE [CaseTrsQueryVersion] 
                            SET 
                                [QDateS] = @QDateS,     
          ...
          Parameter.Clear();
          Parameter.Add(new CommandParameter("@NewID", strVersionNewID));
          Parameter.Add(new CommandParameter("@QDateS", QDateS));
      ...
      public DataSet getDataSetByXMLUser(string txid, string xml, out string c)
      {
        ...
        /// <param name="dt"></param>
        /// <param name="txid"></param>
        /// <param name="BOPS067050SendNewID"></param>

        public void Insert67050V6Send(string VersionNewID, DataTable dt, string txid, string BOPS067050V6SendNewID)
        {
            ...
            using (dbConnection)
            {
                try
                {
                    ...
                    CommandParameterCollection cmdparm = new CommandParameterCollection();
                    string insert67050V6Sql = "";

                    // base.Parameter.Clear();

                    insert67050V6Sql = @" insert BOPS067050V6Send
                                             (
                                             NewID
                                             ,VersionNewID
                                             , CustIdNo
                    ...
                    string chksql = @"select NewID  from BOPS067050V6Send where  SendStatus = '02'  and  VersionNewID = '" + VersionNewID + @"'  and CustIdNo = '" + dt.Rows[0]["CUST_ID_NO"].ToString().Trim() + @"' ";
                    DataTable dtExist = OpenDataTable(chksql);
                    if (!string.IsNullOrEmpty(insert67050V6Sql) && (dtExist.Rows.Count < 1))
                    {
                        base.ExecuteNonQuery(insert67050V6Sql, dbTransaction, false);
                    }
                    dbTransaction.Commit();
        ...
        public void Insert67050V4Send(string VersionNewID, DataTable dt, string txid, string BOPS067050V4SendNewID)
        {
            ...
            using (dbConnection)
            {
                try
                {
                    ...
                    string chksql = @"select NewID  from BOPS067050V4Send where  SendStatus = '02'  and  VersionNewID = '" + VersionNewID + @"'  and CustIdNo = '" + dt.Rows[0]["CUST_ID_NO"].ToString().Trim() + @"' ";
                    DataTable dtExist = OpenDataTable(chksql);
        ...
        public void Insert67050Send(string VersionNewID, DataTable dt, string txid, string BOPS067050SendNewID)
      {
          ...
          using (dbConnection)
          {
              try
              {
                  ...
                  CommandParameterCollection cmdparm = new CommandParameterCollection();
                  string insert67050Sql = "";

                 // base.Parameter.Clear();

                  insert67050Sql = @" insert BOPS067050Send
                                             (
                                             NewID
                                             ,VersionNewID
                                             , CustIdNo
                     ...
                     //  base.Parameter.Add(new CommandParameter("@CreatedUser", "System);

                  string chksql = @"select NewID  from BOPS067050Send where  SendStatus = '02'  and  VersionNewID = '" + VersionNewID + @"'  and CustIdNo = '" + dt.Rows[0]["CUST_ID_NO"].ToString().Trim() + @"' ";
                  DataTable dtExist = OpenDataTable(chksql);
                  if (!string.IsNullOrEmpty(insert67050Sql)  &&  (dtExist.Rows.Count < 1) )
                  {
                      base.ExecuteNonQuery(insert67050Sql, dbTransaction, false);
                  }
                  dbTransaction.Commit();
      ...
      public void Insert081019Send(string VersionNewID, DataTable dt, string txid, string BOPS060490SendNewID,  string parApplyBranch, string parTellerNo,string CustAccount,string Currency)
      {
              ...
              try
              {
                  ...
                  */
                          #region 組BOPS081019Send sql
                              ...
                              //if (Currency != "TWD")
                              //{
                              //    CustAccount = "000" + CustAccount.Substring(0, CustAccount.Length - 3);
                              //}
                          ...
                          base.Parameter.Add(new CommandParameter("@Currency", strCurrency));

                          string chksql = @"select NewID  from BOPS081019Send  where   SendStatus = '02'  and  VersionNewID = '" + VersionNewID + @"' and Currency = '" + strCurrency + @"'  and AcctNo = '" + CustAccount + @"' ";
                          DataTable dtExist = OpenDataTable(chksql);

                      // 新增81019Send
                          if (!string.IsNullOrEmpty(insert081019Sql)  &&  (dtExist.Rows.Count < 1) )
                      {
                  ...
                  if (insert081019Sql != "")
                  {
                      WriteLog("VersionNewID = [" + VersionNewID + "], TxID=[" + txid + "],資料寫入Table: " + "BOPS081019Send");
                  }
              }
              catch (Exception ex)
              {
                  dbTransaction.Rollback();
                  WriteLog("VersionNewID = [" + VersionNewID + "], TxID=[" + txid + "],資料寫入Table: " + "BOPS081019Send"+ex.Message.ToString());
                  throw ex;
              }
          }
               ...
               */
               for (int i = 0; i < 6; i++)
               {
                      ...
                      && dt.Rows[0][strColumnName].ToString().Trim() != "00000000000000000")
                  {
                     ...
                     //DataTable dtOPENEND = new DataTable();
                     //DataTable dtOPENDATE = new DataTable();
                     //string strQDateS = "";
                     //string strQDateE = "";
                      /// 補81019 起訖日期
                     //dtOPENEND = OpenDataTable("SELECT QDateS,QDateE  FROM CaseTrsQueryVersion  where NewID = '" + VersionNewID + "' ");
                     //if (dtOPENEND.Rows.Count > 0)
                     //{
                     //    strQDateS = dtOPENEND.Rows[0]["QDateS"].ToString();
                     //    strQDateE = dtOPENEND.Rows[0]["QDateE"].ToString();
                     //}
                     // /// 如果沒有查詢起日,要去找401RECV 開戶日
                     // if (strQDateS.Length < 8)
                     // {
                     //      dtOPENDATE = OpenDataTable("SELECT OPEN_DATE  FROM BOPS000401Recv where VersionNewID = '" + VersionNewID + "' ");
                     //     if (dtOPENDATE.Rows.Count > 0)
                     //     {
                     //         strQDateS = dtOPENDATE.Rows[0]["OPEN_DATE"].ToString();
                     //     }
                     // }
                     #region 組BOPS081019Send sql
      ...
      public int UpdateQueryVersionStatus(string Status, string NewID)
      {
         ...
         {
            string sql = @"
UPDATE CaseTrsQueryVersion SET Status = @Status  WHERE   NewID = @NewID;
";
            base.Parameter.Clear();
