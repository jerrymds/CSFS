    ...
    public class AgentToHandleBIZ : CommonBIZ
    {
        ...
        public IList<AgentToHandle> GetQueryList(AgentToHandle AToH, int pageIndex, string strSortExpression, string strSortDirection, string EmpId)
        {
            try
            {
                ...
                {
                    sqlWhere += @" and (B.CaseId in (select CaseId from CaseNoChangeHistory  WITH (NOLOCK) where  OldCaseNo like @CaseNo) or CaseNo like @CaseNo) ";
                ...
                sqlStr += @" select row_number() over (partition by CaseId order by caseid ,MemoDate desc ) pid ,MemoDate,CaseId  into #memo from CaseMemo  WITH (NOLOCK) where  MemoType='CaseSeizure'
                            ;with T1 
	                        as
	                        (
                               ...
                               FROM [CaseMaster] AS B  WITH (NOLOCK) 
	                        ...
	                        select a.* from T3 a   WITH (NOLOCK) 
                            drop table #memo ";
        ...
        public string GetManagerID(string userId)
        {
            try
            {
                string strSql = @"SELECT ManagerID FROM LdapEmployee  WITH (NOLOCK) where EmpID=@EmpID";
                base.Parameter.Clear();
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public string AutoPay_ChenHe(List<Guid> aryCaseId, List<string> aryAgentId, string userId)
        {
            if (aryCaseId == null || !aryCaseId.Any() || aryAgentId == null || !aryAgentId.Any()) return "99";
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    foreach (Guid caseId in aryCaseId)
                    {
                        ...
                        CaseMaster masterObj = master.NewMasterModel(caseId, dbTransaction);
        ...
        {
            string sql = @"select limitDate from CaseMaster  WITH (NOLOCK) where CaseId=@CaseId";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
