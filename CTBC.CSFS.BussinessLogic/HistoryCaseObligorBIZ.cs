...
namespace CTBC.CSFS.BussinessLogic
{
	...
	/// 创建日期：2010-02-23
	/// </summary>
	public class HistoryPopupComparer : IEqualityComparer<HistoryCaseObligor>
	{
		public static HistoryPopupComparer Default = new HistoryPopupComparer();
    ...
    public class HistoryCaseObligorBIZ : CommonBIZ
    {
        ...
        public int Create(List<HistoryCaseObligor> model, IDbTransaction trans = null)
        {
  
            int rtn = 0;
            foreach (HistoryCaseObligor obligor in model.Where(obligor => !string.IsNullOrEmpty(obligor.ObligorName) || !string.IsNullOrEmpty(obligor.ObligorNo) ||
                                                                   !string.IsNullOrEmpty(obligor.ObligorAccount)))
            {
                ...
                var strSql = @" insert into History_CaseObligor  (CaseId,ObligorName,ObligorNo,ObligorAccount,CreatedUser,CreatedDate) 
                                        values (
        ...
        public int CreateLog(List<HistoryCaseObligor> model, HistoryCaseMaster cm, IDbTransaction trans = null)
        {
...
//                            values(@CaseId,@CreatedUser,@DocNo,@ObligorNo,'67072','0',GetDate());";
        ...
        public IList<HistoryCaseObligor> GetAllServiceObligorNo()
        {
            ...
            IList<HistoryCaseObligor> list = SearchList<HistoryCaseObligor>(strSql);
            return list;
        }
		...
		public int Edit(List<HistoryCaseObligor> oldList, List<HistoryCaseObligor> newList, IDbTransaction trans = null)
        {
            int rtn = 0;
			for (int i = 0; i < newList.Count; i++)
			{
				...
				{
					if (newList[i].ObligorId != 0 && !HistoryPopupComparer.Default.Equals(oldList[i], newList[i]))
					{
						newList[i].isUpdate = true;
						int updateRtn = 0;
						string strUpdateSql = string.Empty;
						strUpdateSql = @" update History_CaseObligor set ObligorName=@ObligorName, ObligorNo=@ObligorNo,ObligorAccount=@ObligorAccount where CaseId=@CaseId and ObligorId=@ObligorId;";
						Parameter.Clear();
						...
						if (oldList[i].ObligorName != newList[i].ObligorName)
						{
							...
							log.TableName = "HistoryCaseObligor";
							log.DispSrNo = 1;
							log.TableDispActive = "1";
							log.CaseId = newList[i].CaseId.ToString();
							log.CaseNo = master.CaseNo.ToString();
							//List<HistoryCaseObligor> list = HistoryCaseObligor.ObligorModel(model.CaseMaster.CaseId);
					...
					if (newList[i].ObligorId == 0 && (string.IsNullOrEmpty(newList[i].ObligorNo) == true ? "" : newList[i].ObligorNo) == "" && (string.IsNullOrEmpty(newList[i].ObligorName) == true ? "" : newList[i].ObligorName) == "" && (string.IsNullOrEmpty(newList[i].ObligorAccount) == true ? "" : newList[i].ObligorAccount) == "")
					{
						...
						strDeleteSql = @" delete from History_CaseObligor where CaseId=@CaseId and ObligorId=@ObligorId;";
						Parameter.Clear();
						Parameter.Add(new CommandParameter("@CaseId", newList[i].CaseId));
				...
				if (newList[i].ObligorId == 0 && (newList[i].ObligorNo != "" || newList[i].ObligorName != "" || newList[i].ObligorAccount != ""))
				{
					...
					strInsertSql = @" insert into History_CaseObligor  (CaseId,ObligorName,ObligorNo,ObligorAccount,CreatedUser,CreatedDate) 
                                        values (
					...
					log.CaseId = newList[i].CaseId.ToString();
					log.CaseNo = master.CaseNo.ToString();
					List<HistoryCaseObligor> list1 = HistoryCaseObligor.ObligorModel(newList[i].CaseId);
        ...
        public List<HistoryCaseObligor> ObligorModel(Guid caseId, IDbTransaction trans = null)
        {
			string strSql = @";with  T1 as(select *, ObligorNo + '-' + ObligorName as ObligorNoAndName 
									from History_CaseObligor where CaseId=@CaseId) ,
									T2 as (select ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from History_CaseDataLog c1,History_CaseObligor where c1.CaseId=@CaseId and TabName='公文資訊-義(債)務人資訊'
                                    and TableName='HistoryCaseObligor' and LinkDataKey=cast(ObligorId as nvarchar(100) )
								   ...
								   from T1)
								   select * from T3";
			//string strSql = "select *, ObligorNo + '-' + ObligorName as ObligorNoAndName from History_CaseObligor where CaseId=@CaseId order by ObligorId desc";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@CaseId", caseId));
            try
            {
                List<HistoryCaseObligor> itemlist = new List<HistoryCaseObligor>();
                IList<HistoryCaseObligor> list = trans == null ? SearchList<HistoryCaseObligor>(strSql) : SearchList<HistoryCaseObligor>(strSql, trans);
                foreach (var item in list)
                {
                    itemlist.Add(item);
                }
        ...
        public HistoryCaseObligor ObligorModelInfo(Guid CaseId)
        {
            string strSql = "select top 1 ObligorNo,ObligorName from [dbo].[History_CaseObligor] where Caseid=@CaseId";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@CaseId", CaseId));
            try
            {
                IList<HistoryCaseObligor> list = base.SearchList<HistoryCaseObligor>(strSql);
                if (list != null) return list[0];
                else return new HistoryCaseObligor();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public bool EditAllServiceHistoryCaseObligor(string status, string tailNum)
        {
        ...
        public IList<HistoryCaseObligor> GetObligorNo99(string serviceName, string tailNum)
        {
        ...
        public IList<HistoryCaseObligor> GetObligor(string ObligorNo)
        {
            try
            {
        ...
        public IList<HistoryCaseObligor> GetObligorNo(string serviceName)
        {
        ...
        public bool EditHistoryCaseObligor(string id, string status, string errorMsg)
        {
