            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    foreach (Guid caseId in aryCaseId)
                    {
                        ...
                        string sqlAgentUser = @"select AgentUser from CaseMaster where CaseId = @CaseId";//IR-2004
                        base.Parameter.Clear();//IR-2004
                        base.Parameter.Add(new CommandParameter("@CaseId", caseId));//IR-2004
                        string AgentUser2 = base.ExecuteScalar(sqlAgentUser, dbTransaction) == null ? userId : base.ExecuteScalar(sqlAgentUser, dbTransaction).ToString();//IR-2004

                        CaseMaster masterObj = master.MasterModel(caseId, dbTransaction);
                        strSql = masterObj.AfterSeizureApproved > 0
                            ? "UPDATE [CaseMaster] SET [AgentSubmitDate2] = GETDATE(),[AgentUser2]=@AgentUser2 WHERE [CaseId] = @CaseId; "//? "UPDATE [CaseMaster] SET [AgentSubmitDate2] = GETDATE(),[AgentUser2]=@userId WHERE [CaseId] = @CaseId; "//IR-2004
                            : "UPDATE [CaseMaster] SET [AgentSubmitDate] = GETDATE() WHERE [CaseId] = @CaseId; ";

                        Parameter.Clear();
                        Parameter.Add(new CommandParameter("CaseId", caseId));
                        Parameter.Add(new CommandParameter("UserId", userId));
                        Parameter.Add(new CommandParameter("AgentUser2", AgentUser2));//IR-2004
                        ExecuteNonQuery(strSql, dbTransaction);
