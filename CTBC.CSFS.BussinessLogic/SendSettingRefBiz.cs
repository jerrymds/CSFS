...
namespace CTBC.CSFS.BussinessLogic
{
    public class SendSettingRefBiz : CommonBIZ
    {
        ...
        public SendSettingRef GetChangeSubjectAndDescription(Guid caseId, string template, string sendKind, int serialId,IDbTransaction trans = null, CasePayeeSetting payeeModel = null)
        {
            string caseKind1 = "";
            string caseKind2 = "";
            ...
            if (caseKind2 == @"支付電子回文")
            {
                // ○○○來自公文資訊「義(債)務人戶名」，若為多名義(債)務人時，請鍵入公文資訊第一個戶名+等，例：王小明等
                List<CaseObligor> listObligor = new CaseObligorBIZ().ObligorModel(caseId, trans);

                string obligs = string.Empty;
                if (listObligor != null && listObligor.Any())
                {
                    obligs = listObligor.First().ObligorName;
                    if (listObligor.Count() > 1)
                    {
                        obligs += "等";
                    }
                    strSubject = strSubject.Replace("{obligor}", obligs);
                }

                strDesc = strDesc.Replace("{GovUnit}", master.GovUnit);
                ...
                strDesc = strDesc.Replace("{GovDate}", iyy.ToString() + "年" + strmm + "月" + strdd + "日");
            ...
            if (caseKind2 == "支付" || caseKind2 == "支付電子回文")
            {
                #region 支付
                ...
                if (listObligor != null && listObligor.Any())
                {
                    ...
                    {
                        strObligorList = strObligorList + "義務人戶名：" + obligor.ObligorName + "\r\n";
                        strObligorList = strObligorList + "統一編號或身分證字號：" + obligor.ObligorNo + "\r\n\r\n";
                    }
                }
                ...
                if (payeeModel == null)
                {
                    if (master.ReceiveKind == "電子公文")
                    {
                        payeeModel = new CasePayeeSettingBIZ().GetEdit(new CasePayeeSetting { CaseId = caseId },serialId, trans).FirstOrDefault();
                    }
                    else
                    {
        ...
        public SendSettingRef DeadGetSubjectAndDescription(DataRow dw,Guid caseId, string template, string sendKind, decimal TotalSeiAmt, IDbTransaction trans = null, CasePayeeSetting payeeModel = null)
        {
            string caseKind1 = "";
            string caseKind2 = "";
            CaseMaster master = new CaseMasterBIZ().MasterModel(caseId, trans);
            DataTable dtObliger = new CaseMasterBIZ().GetCaseObligorNameByCaseId(caseId);
            string strObligerName = "";
            if (dtObliger.Rows.Count > 0)
            {
                for (int i = 0; i < dtObliger.Rows.Count; i++)
                {
                    if (dtObliger.Rows[i]["ObligorName"].ToString() != "")
                    {
                        strObligerName = strObligerName + dtObliger.Rows[i]["ObligorName"].ToString();                        
                    }
            ...
            string sendDate = master.PayDate;
            caseKind1 = @"外來文案件";
            caseKind2 = master.CaseKind2 == "國稅局死亡" ? @"國稅局死亡" : master.CaseKind2 == Lang.csfs_property_declaration1 ? Lang.csfs_property_declaration1 : @"國稅局死亡";
            SendSettingRefBiz refBiz = new SendSettingRefBiz();
            SendSettingRef refObj = refBiz.GetSendSettingRef(caseKind1, caseKind2, trans);
            if (refObj == null)
                return new SendSettingRef();
            ...
            string strDesc = refObj.Description;
            //* 公文
            strSubject = strSubject.Replace("{GovUnit}", master.GovUnit);
            strSubject = strSubject.Replace("{GovNo}", master.GovNo);
            int iyy = Convert.ToInt16(master.GovDate.Substring(0, 4)) - 1911;
            string strmm = master.GovDate.Substring(5, 2);
            string strdd = master.GovDate.Substring(8, 2);
            ...
            strSubject = strSubject.Replace("{CaseNo}", master.CaseNo);
            strDesc = strDesc.Replace("{CaseNo}", master.CaseNo);
            if (master.CaseKind == "外來文案件" && master.CaseKind2 == "國稅局死亡")
            {
                strDesc = strDesc.Replace("{ObligorNo}",dw[5].ToString());
                strDesc = strDesc.Replace("{InCharge}", strObligerName);
                if (dw[22].ToString()== "Y" || dw[22].ToString() == "y")
                {
                    strDesc = strDesc.Replace("{deposit}", "有");
                }
                else
                {
                    strDesc = strDesc.Replace("{deposit}", "無");
                }
                if (dw[23].ToString() == "Y" || dw[23].ToString() == "y")
                {
                    strDesc = strDesc.Replace("{Loan}", "有");
                }
                else
                {
                    strDesc = strDesc.Replace("{Loan}", "無");
                }
                if (dw[24].ToString() == "Y" || dw[24].ToString() == "y" || dw[28].ToString() == "Y" || dw[28].ToString() == "y" )
                {
                    strDesc = strDesc.Replace("{LoanMgr}", "有");
                }
                else
                {
                    strDesc = strDesc.Replace("{LoanMgr}", "無");
                }
                string strBox = "無";
                if (dw[26].ToString().Trim() != "N")
                {
                    strBox = dw[26].ToString().Trim()+"分行有";
                }
                //string strBox = dw[26].ToString().Replace("Y", "有");
                //strBox = strBox.Replace("N", "無");                strDesc = strDesc.Replace("{Box}", strBox);
                if (dw[27].ToString() == "Y" || dw[27].ToString() == "y")
                {
                    strDesc = strDesc.Replace("{CreditCard}", "有");
                }
                else
                {
                    strDesc = strDesc.Replace("{CreditCard}", "無");
                }
            }
            //strDesc = strDesc.Replace("{GovDate}", master.GovDate);
            DateTime dt = Convert.ToDateTime(master.GovDate);
        ...
        public SendSettingRef AutoGetSubjectAndDescription(Guid caseId, string template, string sendKind, decimal TotalSeiAmt, IDbTransaction trans = null, CasePayeeSetting payeeModel = null)
        {
            string caseKind1 = "";
            string caseKind2 = "";
            CaseMaster master = new CaseMasterBIZ().MasterModel(caseId, trans);
