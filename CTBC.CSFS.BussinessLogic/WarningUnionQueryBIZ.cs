...
{
    public class WarningUnionQueryBIZ : CommonBIZ
    {
        public WarningUnionQueryBIZ(AppController appController)
            : base(appController)
        { }

        public WarningUnionQueryBIZ()
        { }

        public IList<WarningUnionQuery> GetQueryList(WarningUnionQuery wq)
        {
            try
            {
                string sqlStr = "";
                ...
                if (!string.IsNullOrEmpty(wq.COL_PID))
                {
                    sqlStrWhere += @" AND COL_PID like @COL_PID";
                    base.Parameter.Add(new CommandParameter("@CustId", "%" + wq.COL_PID.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(wq.COL_ACCOUNT2))
                {
                    sqlStrWhere += @" AND COL_ACCOUNT2 like @COL_ACCOUNT2";
                    base.Parameter.Add(new CommandParameter("@COL_ACCOUNT2", "%" + wq.COL_ACCOUNT2.Trim() + "%"));
                }
                ...
                if (!string.IsNullOrEmpty(wq.COL_165CASE))
                {
                    sqlStrWhere += @" AND COL_165CASE like @COL_165CASE";
                    base.Parameter.Add(new CommandParameter("@COL_165CASE", "%" + wq.COL_165CASE.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(wq.Set))
                {
                    sqlStrWhere += @" AND [Set] like @Set";
                    base.Parameter.Add(new CommandParameter("@Set", wq.Set.Trim()));
                }
                if (!string.IsNullOrEmpty(wq.ModifyDateS))
                {
                    sqlStrWhere += @" AND CreatedDate >= @ModifyDateS";
                    base.Parameter.Add(new CommandParameter("@ModifyDateS", wq.ModifyDateS));
                }
                if (!string.IsNullOrEmpty(wq.ModifyDateE))
                {
                    string ModifyDateE = UtlString.FormatDateString(Convert.ToDateTime(wq.ModifyDateE.Replace('/', ' ').ToString()).AddDays(1).ToString("yyyyMMdd"));
                    sqlStrWhere += @" AND ForCDate < @ModifyDateE ";
                    base.Parameter.Add(new CommandParameter("@ModifyDateE", ModifyDateE));
                }
                ...
                sqlStr += @";with T1 
	                        ...
	                        (
select T1.SerialNo,wd.SerialID, wm.DocNo,wm.currency,CustId,CustAccount,BankID,AccountStatus,Retry,wd.Status,
...
case ISNULL(ws.NoClosed,0)
	...
	inner join T1 on T1.SerialID = wd.SerialID
    ...
    WHERE wd.Status = 'C01'  " + sqlStrWhere + @") select T1.SerialNo,t2.* from t2 inner join T1
           on t2.SerialID = t1.SerialID
           ORDER BY t2.DocNo ASC, ForCDate ASC  ";
                //Add by zhangwei 20180315 end
 //               WHERE wd.Status = 'C01'  " + sqlStrWhere + @" ) 
	//select* from t2 ORDER BY DocNo ASC,ForCDate ASC ";
                IList<WarningUnionQuery> _ilsit = base.SearchList<WarningUnionQuery>(sqlStr);
        ...
        public WarningUnionQuery GetWarningUnionQueryListByDocNo(string COL_C1003CASE)
        {
            string sql = @"SELECT * FROM [WarningUnionQuery]  where COL_C1003CASE = @COL_C1003CASE ";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@COL_C1003CASE", COL_C1003CASE));
            IList<WarningUnionQuery> list = base.SearchList<WarningUnionQuery>(sql);
            if (list != null && list.Count > 0) return list[0];
            else return new WarningUnionQuery();
        }
        public DataTable ExcelList(WarningUnionQuery wq)
        {
            string sqlStrWhere = "";
            if (!string.IsNullOrEmpty(wq.COL_PID))
            {
            ...
            if (!string.IsNullOrEmpty(wq.COL_ACCOUNT2))
            {
                sqlStrWhere += @" AND CustAccount like @COL_ACCOUNT2";
                base.Parameter.Add(new CommandParameter("@COL_ACCOUNT2", "%" + wq.COL_ACCOUNT2.Trim() + "%"));
            }
            if (!string.IsNullOrEmpty(wq.DocNo))
            {
            ...
            {
                string ModifyDateE = UtlString.FormatDateString(Convert.ToDateTime(wq.ModifyDateE.Replace('/', ' ').ToString()).AddDays(1).ToString("yyyyMMdd"));
                sqlStrWhere += @" AND CreatedDate < @ModifyDateE ";
                base.Parameter.Add(new CommandParameter("@ForCDateE", ModifyDateE));
            }
        ...
        //public MemoryStream ParmCodeExcel_NPOI(WarningUnionQuery wq)
        //{
  ...
  on wm.DocNo like '%'+wg.CHQ_PAYEE+'%' where wm.docno = @DocNo ";
        ...
        public MemoryStream ParmCodeExcel_NPOI(WarningUnionQuery model)
        {
            ...
            IList<WarningUnionQuery> ilst = GetQueryList(model);
            ...
            if (ilst != null)
            {
                ms = ExcelExport(ilst, headerColumns,
                                                   delegate (HSSFRow dataRow, WarningUnionQuery dataItem)
                                                   {
                                                       ...
                                                       dataRow.CreateCell(5).SetCellValue(dataItem.COL_PID);
                                                       dataRow.CreateCell(6).SetCellValue(dataItem.COL_ACCOUNT2);
                                                       dataRow.CreateCell(7).SetCellValue(dataItem.COL_OTHERBANKID);
                                                       //dataRow.CreateCell(8).SetCellValue(UtlString.FormatDateTw(dataItem.CreatedDate));
                                                       ...
                                                       dataRow.CreateCell(20).SetCellValue(dataItem.COL_165CASE);//165案號
                                                   });
            }
            return ms;
        }
