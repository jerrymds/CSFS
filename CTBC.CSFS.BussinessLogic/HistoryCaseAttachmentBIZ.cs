...
{
    public class HistoryCaseAttachmentBIZ : CommonBIZ
    {
        ...
        public int Create(HistoryCaseAttachment model, IDbTransaction trans = null)
        {
            string strSql = @" insert into History_CaseAttachment  (CaseId,AttachmentName,AttachmentServerPath,AttachmentServerName,isDelete,CreatedUser,CreatedDate) 
                                        values (
        ...
        public int Edit(IList<HistoryCaseAttachment> model, IDbTransaction trans = null)
        {
            string strSql = string.Empty;
            foreach (HistoryCaseAttachment caseAttach in model.Where(caseAttach => caseAttach.AttachmentName != null && caseAttach.AttachmentServerName != null && caseAttach.AttachmentServerPath != null).Where(caseAttach => caseAttach.AttachmentName != "" && caseAttach.AttachmentServerName != "" && caseAttach.AttachmentServerPath != ""))
            {
                strSql += @" insert into History_CaseAttachment  (CaseId,AttachmentName,AttachmentServerPath,AttachmentServerName,isDelete,CreatedUser,CreatedDate) 
                                        values (
        ...
        public List<HistoryCaseAttachment> AttachmentList(Guid CaseId, IDbTransaction trans = null)
        {
            string strSql = "select * from History_CaseAttachment where CaseId=@CaseId";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@CaseId", CaseId));
            try
            {
                return trans == null ? base.SearchList<HistoryCaseAttachment>(strSql).ToList() : base.SearchList<HistoryCaseAttachment>(strSql, trans).ToList();

                //List<HistoryCaseAttachment> listItem = new List<HistoryCaseAttachment>();
                //IList<HistoryCaseAttachment> list = base.SearchList<HistoryCaseAttachment>(strSql);
        ...
        public HistoryCaseAttachment GetAttachmentInfo(int attachId)
        {
            string strSql = "SELECT [AttachmentId],[CaseId],[AttachmentName],[AttachmentServerPath],[AttachmentServerName],[isDelete],[CreatedUser],[CreatedDate] " +
                            "FROM [History_CaseAttachment] " +
                            "WHERE [AttachmentId]=@AttachmentId";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@AttachmentId", attachId));
            IList<HistoryCaseAttachment> list = SearchList<HistoryCaseAttachment>(strSql);
            return list.FirstOrDefault();
        }
        public int DeleteAttatch(string AttatchId)
        {
            string strSql = " delete from History_CaseAttachment where AttachmentId=@AttachmentId ";
            base.Parameter.Clear();
            ...
            try
            {
				...
				HistoryCaseAttachment attachment = GetAttachmentInfo(int.Parse(AttatchId));
