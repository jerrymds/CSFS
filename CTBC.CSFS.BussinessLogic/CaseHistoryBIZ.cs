...
{
    public class CaseHistoryBIZ : CommonBIZ
    {
        ...
        public List<CaseHistory> getHistoryByCaseId(Guid caseId)
        {
            string strSql = "select * from CaseHistory where caseId = @CaseId";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@CaseId", caseId.ToString()));
            return base.SearchList<CaseHistory>(strSql).ToList() ?? new List<CaseHistory>();
        }

        public bool insertCaseHistory(CaseHistory item,IDbTransaction dbTransaction = null)
        {
            string strSql = "insert into CaseHistory (CaseId,FromRole,FromUser,FromFolder,Event,EventTime,ToRole,ToUser,ToFolder,CreatedUser,CreatedDate) values "+
        ...
        public bool insertCaseHistory(Guid caseId, string action, string userId, IDbTransaction dbTransaction = null)
        {
            ...
            CaseHistory old = getHistoryByCaseId(caseId).OrderByDescending(m => m.HistoryId).FirstOrDefault();
        ...
        {
            List<CaseHistory> list = getHistoryByCaseId(caseId);
            ...
            if (list != null)
            {
                ms = ExcelExport(list, headerColumns,
                                               delegate(HSSFRow dataRow, CaseHistory dataItem)
                                               {
