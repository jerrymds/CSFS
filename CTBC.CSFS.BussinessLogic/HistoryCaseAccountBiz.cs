...
{
    public class HistoryCaseAccountBiz : CommonBIZ
    {
        #region 扣押
		...
		/// <returns></returns>
		public IList<HistoryCaseSeizure> GetCaseSeizure(Guid caseId, IDbTransaction trans = null)
		{
			string strSql = @";with  T1 as(SELECT s.[SeizureId],s.[CaseId],s.[PayCaseId],s.[CaseNo],s.[CustId],s.[CustName],s.[BranchNo],s.[BranchName],s.[Account],s.[AccountStatus],s.[Currency],s.[Balance],
                            ...
                            from History_CaseSeizure s  
                            left join History_CaseMaster m on s.CaseId=m.CaseId
                            ...
                            WHERE History_TX_60491_Grp.CaseID = @CaseId 
                            --and [CustomerId] IN (SELECT [ObligorNo] FROM [History_CaseObligor] WHERE [CaseId] = @CaseId)
                            ) h on h.CustomerId = s.CustId                             
                            left join History_TX_60491_Detl d on s.CustId = d.CUST_ID and s.Account = d.Account and d.FKSNO = h.SNO
							--inner join History_CaseObligor g on g.CaseId =m.CaseId and g.ObligorNo = s.CustId
                            WHERE s.[CaseId] = @CaseId),
							T2 as (select distinct  TXType,ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from History_CaseDataLog c1,CaseSeizure
							where c1.CaseId=@CaseId and TabName='扣押設定' and TableName='CaseSeizure' and LinkDataKey=cast(SeizureId as nvarchar(100) )
								and TXDateTime=(select max(TXDateTime) from History_CaseDataLog c2 where c1.CaseId=c2.CaseId and c1.TabName=c2.TabName and c1.TableName=c2.TableName and c1.ColumnID=c2.ColumnID and c1.LinkDataKey=c2.LinkDataKey)),
							T3 as (select T1.*,
			...
			return trans == null ? SearchList<HistoryCaseSeizure>(strSql) : SearchList<HistoryCaseSeizure>(strSql, trans);
		}

		public HistoryCaseSeizure GetCaseSeizureInfo(Guid caseId, int seizureId)
		{
			string strSql = @"select * from History_CaseSeizure where CaseId = @CaseId and SeizureId=@SeizureId";
			Parameter.Clear();
			Parameter.Add(new CommandParameter("@CaseId", caseId));
			Parameter.Add(new CommandParameter("@SeizureId", seizureId));
			return SearchList<HistoryCaseSeizure>(strSql)[0];
		}

		//新增操作
        public void Add(Guid CaseId, HistoryCaseSeizure formItem, Guid TXSNO, DateTime TXDateTime, string TXType = null)
		{
		...
		//刪除操作
		public void Delete(Guid CaseId, HistoryCaseSeizure formItem, Guid TXSNO, DateTime TXDateTime)
		{ 
			CaseDataLog log = new CaseDataLog();
		...
		public IList<HistoryCaseSeizure> GetCaseSeizureFromTx(Guid caseId)
		{
            ...
            string strSql = @";with  T1 as(SELECT @CaseId AS [CaseId],M.[CustomerId] AS [CustId],M.[CustomerName] AS [CustName],
							...
							LEFT OUTER JOIN [History_TX_60491_Detl] AS D ON M.[SNO] = D.[FKSNO]
							LEFT OUTER JOIN [PARMCode] AS P ON P.[CodeType]= 'RCAF_BRANCH' AND P.[CodeNo] = D.[Branch]
							LEFT OUTER JOIN History_CaseSeizure as S on D.CaseId=S.CaseId
							--inner join History_CaseObligor g on g.CaseId =@CaseId and g.ObligorNo = M.CustomerId
							WHERE M.RowID = 1) ,
							T2 as (select distinct ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from History_CaseDataLog c1,CaseSeizure
							where c1.CaseId=@CaseId and TabName='扣押設定' and TableName='CaseSeizure' and LinkDataKey=cast(SeizureId as nvarchar(100) )
                            and TXDateTime=(select max(TXDateTime) from History_CaseDataLog c2 where c1.CaseId=c2.CaseId and c1.TabName=c2.TabName and c1.TableName=c2.TableName and c1.ColumnID=c2.ColumnID and c1.LinkDataKey=c2.LinkDataKey)),
							T3 as (select T1.*,
							(select case  when ColumnValueBefore<>CustId then 'true' else 'false' end from T2 where ColumnID='CustId' and LinkDataKey=cast(SeizureId as nvarchar(100) )) as CustIdflag,
			...
			Parameter.Clear();
			Parameter.Add(new CommandParameter("@CaseId", caseId));
			IList<HistoryCaseSeizure> list = base.SearchList<HistoryCaseSeizure>(strSql);


            HistoryCaseMaster master = new HistoryCaseMasterBIZ().MasterModel(caseId);
			HistoryCustomerInfoBIZ custBiz = new HistoryCustomerInfoBIZ();
			...
			{
				//* 至少主機下來有查到資料
				foreach (HistoryCaseSeizure seizure in list)
				{
					...
					{
                        CTBC.CSFS.Models.HistoryTX_33401 tx33401 = custBiz.GetLatestTx33401(seizure.Account, caseId, seizure.Currency);//adam 20160427
						if (tx33401 != null)
						{
        ...
        public bool SaveSeizureSetting(List<HistoryCaseSeizure> listForm, string memo, Guid CaseId)
        {
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    IList<HistoryCaseSeizure> listOld = GetCaseSeizure(CaseId, null);
					...
					if (listForm != null)
					{
						foreach (HistoryCaseSeizure ss in listForm)
						{
					...
					{
						listForm = new List<HistoryCaseSeizure>();
					}
                    ...
                    #endregion
                    foreach (HistoryCaseSeizure formItem in listForm)
					{
						...
						{
                            HistoryCaseSeizure cs = GetCaseSeizureInfo(formItem.CaseId, formItem.SeizureId);
        ...
        public HistoryCaseEdocFile OpenExcel(Guid caseId)
        {
            //caseId = new Guid("635FEDA7-59DA-478F-99DE-77F28FFF1F63");
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='xlsx' and Type='歷史'";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
            return list.FirstOrDefault();
        }
        public HistoryCaseEdocFile OpenPdf(Guid caseId)
        {
            //caseId = new Guid("635FEDA7-59DA-478F-99DE-77F28FFF1F63");
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='pdf' and Type='歷史'";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
            return list.FirstOrDefault();
        ...
        public HistoryCaseEdocFile OpenTxtDoc(Guid caseId)
        {
            //caseId = new Guid("635FEDA7-59DA-478F-99DE-77F28FFF1F63");
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='txt' and Type='收文'";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
            return list.FirstOrDefault();
        }
        public HistoryCaseEdocFile OpenTxtDoc(string caseId,string fName)
        {
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId  and fileName=@filename";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
        ...
        public HistoryCaseEdocFile OpenHisDoc(string caseId, string fName,int rownumber)
        {
            string strSql = @"WITH downfile AS  
(  
select CaseId, Type, FileType, FileName, FileObject, SendNo,ROW_NUMBER() OVER (ORDER BY CaseId) AS RowNumber from History_CaseEdocFile where CaseId=@CaseId   
)
select CaseId, Type, FileType, FileName, FileObject, SendNo from downfile   where CaseId=@CaseId  and fileName=@filename and RowNumber=@RowNumber ";
        ...
        public HistoryCaseEdocFile OpenPdfDoc(string caseId)
		{
			//caseId = "08F4A94D-5AA7-4010-9304-6AB97D011413";
			string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='pdf' and Type='收文'";
			Parameter.Clear();
			Parameter.Add(new CommandParameter("@CaseId", caseId));
			List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
			return list.FirstOrDefault();
		}
        public HistoryCaseEdocFile OpenPayPdfDoc1(string caseId)
        {
            //caseId = "08F4A94D-5AA7-4010-9304-6AB97D011413";
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='pdf' and Type='收文' and UPPER(filename) like '%_DI%' ";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
            return list.FirstOrDefault();
        }
        public HistoryCaseEdocFile OpenPayPdfDoc2(string caseId)
        {
            //caseId = "08F4A94D-5AA7-4010-9304-6AB97D011413";
            string strSql = @"select * from History_CaseEdocFile where CaseId=@CaseId and FileType='pdf' and Type='收文' and UPPER(filename) like '%ATTCH1%'";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            List<HistoryCaseEdocFile> list = base.SearchList<HistoryCaseEdocFile>(strSql).ToList();
            return list.FirstOrDefault();
        ...
        public List<HistoryBatchQueue> StatusList(Guid CaseId)
        {
            //string strSql = @" select Status from BatchQueue where CaseId=@CaseId";
            string strSql = @" select * from History_BatchQueue where CaseId=@CaseId 
                                  union  
                                select * from History_autolog where CaseId=@CaseId ";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", CaseId));
            List<HistoryBatchQueue> list = base.SearchList<HistoryBatchQueue>(strSql).ToList();
            if (list != null && list.Count > 0) return list;
            else return new List<HistoryBatchQueue>();
        ...
        //*新增案件及新增扣押维护
        public JsonReturn SaveSeizureSetting(List<HistoryCaseSeizure> listForm)
        {
            Guid strCaseId = new Guid();
            HistoryCaseNoTableBIZ noBiz = new HistoryCaseNoTableBIZ();
            HistoryCaseMasterBIZ master = new HistoryCaseMasterBIZ();
            IDbConnection dbConnection = OpenConnection();
            IDbTransaction dbTrans = null;
            bool bFlag = true;
            try
            {
                using (dbConnection)
                {
                    ...
                    if (listForm != null && listForm.Any())
                    {
                        HistoryCaseMaster model = new HistoryCaseMaster();
                        model.GovKind = "法院";
                        model.GovUnit = "扣押補建";
        ...
        public HistoryCaseSeizure GetCaseSeizure(string SeizureId, IDbTransaction dbtrans = null)
        {
            string strSql = @" select s.SeizureId,s.CaseId,s.CaseNo,s.Currency,s.CustId,s.CustName,s.BranchNo,s.BranchName,s.Account,s.SeizureAmount,s.CancelAmount,s.AccountStatus,s.PayAmount,s.TripAmount,
                                        s.ExchangeRate,s.SeizureAmountNtd from   HistoryCaseSeizure s
            ...
            //return list.FirstOrDefault();
            return dbtrans == null ? SearchList<HistoryCaseSeizure>(strSql).FirstOrDefault() : SearchList<HistoryCaseSeizure>(strSql, dbtrans)[0];
        }

        public bool SeizureMaintainEdit(HistoryCaseSeizure model)
        {
        ...
        {
            string strSql = "DELETE  History_CaseSeizure WHERE " +
                            "CaseId = @CaseId " +
        ...
        {
            string strSql = "DELETE  History_CaseSeizure WHERE  CaseId = @CaseId ";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("CaseId", caseId));
            return dbtrans == null ? ExecuteNonQuery(strSql) > 0 : ExecuteNonQuery(strSql, dbtrans) > 0;
        }
        ...
        public bool InsertSeizureSetting(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
            item.SeizureSeq = GetMaxSeqByCaseId(item.CaseId);
            string strSql = @"INSERT INTO [ History_CaseSeizure]
        ...
        public bool UpdateSeizureBalance(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
        ...
        public bool UpdateSeizureSetting(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
            string strSql = @"UPDATE [History_CaseSeizure]
                            SET 
                                [CustId]=@CustId,
                                [CustName]=@CustName,
                                [BranchNo]=@BranchNo,
        ...
        /// <param name="caseId"></param>
        /// <returns></returns>
        public IList<HistoryCaseSeizure> GetCaseSeizureWithoutPaySetting(Guid caseId)
        {
            IList<HistoryCaseSeizure> list = GetCaseSeizure(caseId);
            List<HistoryCaseSeizure> list2 = list != null ? list.ToList() : new List<HistoryCaseSeizure>();
            list2.RemoveAll(m => m.SeizureStatus != SeizureStatus.AfterSeizureSetting);
            return list2;
        }

        public IList<HistoryCaseSeizure> GetCaseSeizureWithEPaySetting(Guid caseId, IDbTransaction trans = null)
        {
            string strSql = ";with  T1 as(SELECT " +
                            ...
                            "FROM [History_CaseSeizure] s left join History_CaseMaster m on s.CaseId=m.CaseId " +
                            "left join  (SELECT top 1 ROW_NUMBER() OVER (PARTITION BY [CustomerId] ORDER BY [SNO] DESC) AS RowID, [SNO],[CustomerId],[CustomerName] FROM [History_TX_60491_Grp] " +
                            "WHERE History_TX_60491_Grp.CaseID = @CaseId) h on h.CustomerId = s.CustId " +
                            "left join History_TX_60491_Detl d on s.CustId = d.CUST_ID and s.Account = d.Account and d.FKSNO = h.SNO " +
                            "WHERE s.[PayCaseId] = @CaseId AND ( s.[SeizureStatus] = '1' or  s.[SeizureStatus] = '3')  " +
                            " and ((len(s.[CustId]) >= 10 and SUBSTRING(s.[CustId],1,10) IN (SELECT [ObligorNo] FROM [History_CaseObligor] WHERE CaseId = @CaseId)) or " +
                            "(len(s.[CustId]) < 10 and SUBSTRING(s.[CustId],1,8) IN (SELECT [ObligorNo] FROM [History_CaseObligor] WHERE CaseId = @CaseId))) " +
                            ") ,T2 as (select ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from History_CaseDataLog c1,CaseSeizure " +
                            "where c1.CaseId=@CaseId and TabName='支付設定' and TableName='CaseSeizure' and LinkDataKey=cast(SeizureId as nvarchar(100) ) " +
                            "and TXDateTime=(select max(TXDateTime) from History_CaseDataLog c2 where c1.CaseId=c2.CaseId and c1.TabName=c2.TabName and c1.TableName=c2.TableName and c1.LinkDataKey=c2.LinkDataKey))," +
                            "T3 as (select T1.*, " +
                            "(select case  when ColumnValueBefore<>AccountStatus then 'true' else 'false' end from T2 where ColumnID='AccountStatus' and LinkDataKey=cast(SeizureId as nvarchar(100) )) as AccountStatusflag," +
        ...
        /// <param name="trans"></param>
        /// <returns></returns>
        public IList<HistoryCaseSeizure> GetCaseSeizureWithPaySetting(Guid caseId, IDbTransaction trans = null)
        {
            string strSql = ";with  T1 as(SELECT " +
                            "s.[SeizureId],s.[CaseId],s.[PayCaseId],s.[CaseNo],s.[CustId],s.[CustName],s.[BranchNo],s.[BranchName],s.[Account],s.[AccountStatus],s.[Currency],s.[Balance], " +
                            "s.[SeizureAmount],s.[ExchangeRate],s.[SeizureAmountNtd],s.[PayAmount],s.[TripAmount],s.[SeizureStatus],s.[CreatedUser],s.[CreatedDate],s.[ModifiedUser],s.[ModifiedDate] " +
        ...
        /// <param name="caseId"></param>
        /// <returns></returns>
        public IList<HistoryCaseSeizure> GetCaseSeizureWithoutPaySettingByCustomerList(Guid caseId)
        {
            string strSql = "SELECT " +
                            ...
                            ",Convert(Nvarchar(10), m.GovDate,111) as GovDate,m.GovNo,m.GovUnit,ISNULL(d.FKSNO,9999999) as FKSNO,ISNULL(d.SNO,9999999) as SNO from  History_CaseSeizure s left join  History_CaseMaster m on s.CaseId=m.CaseId " +
                            "left join  (SELECT top 1 ROW_NUMBER() OVER (PARTITION BY [CustomerId] ORDER BY [SNO] DESC) AS RowID, [SNO],[CustomerId],[CustomerName] FROM [History_TX_60491_Grp] " +
                            "WHERE  History_TX_60491_Grp.CaseID = @CaseId) h on h.CustomerId = s.CustId " +
                            "left join  History_TX_60491_Detl d on s.CustId = d.CUST_ID and s.Account = d.Account and d.FKSNO = h.SNO " +
                            "WHERE ((len(s.[CustId]) >= 10 and SUBSTRING(s.[CustId],1,10) IN (SELECT [ObligorNo] FROM [History_CaseObligor] WHERE CaseId = @CaseId)) or " +
                            "(len(s.[CustId]) < 10 and substring(s.[CustId],1,8) IN (SELECT [ObligorNo] FROM [History_CaseObligor] WHERE CaseId = @CaseId))) " +
                            "AND s.[SeizureStatus] = @SeizureStatus   order by  m.CaseNo,m.GovDate,m.GovUnit,s.Account";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            Parameter.Add(new CommandParameter("@SeizureStatus", SeizureStatus.AfterSeizureSetting));
            return SearchList<HistoryCaseSeizure>(strSql);
        }
        ...
        /// <returns></returns>
        public bool SavePaySetting(List<HistoryCaseSeizure> saveList, Guid CaseId)
        {
            IDbConnection dbConnection = OpenConnection();
            IDbTransaction dbTransaction = null;
            bool rtn = true;
            try
            {
                using (dbConnection)
                {
                    ...
                    dbTransaction = dbConnection.BeginTransaction();
                    IList<HistoryCaseSeizure> listOld = GetCaseSeizureWithPaySetting(CaseId, dbTransaction);
                    if (saveList != null && saveList.Any())
                    {
                        foreach (HistoryCaseSeizure formItem in saveList)
                        {
                            HistoryCaseSeizure cs = GetCaseSeizure(formItem.SeizureId.ToString(), dbTransaction);
                            rtn = rtn && UpdatePaySetting(formItem, dbTransaction);
                            if (listOld.Count > 0)
                            {
                                #region 狀態
                    ...
                    {
                        foreach (HistoryCaseSeizure oldItem in listOld.Where(a => saveList == null || !saveList.Exists(b => b.SeizureId == a.SeizureId)))
                        {
        ...
        /// <param name="trans"></param>
        /// <returns></returns>
        public bool UpdatePaySetting(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
            string strSql = "UPDATE [History_CaseSeizure] " +
        ...
        public bool ClearPaySetting(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
            string strSql = "UPDATE [CaseSeizure] " +
                            " SET [PayCaseId] = @PayCaseId , " +
        ...
        /// <param name="caseId"></param>
        /// <returns></returns>
        public IList<HistoryCaseSeizure> GetCaseSeizureWithCancel(Guid caseId, IDbTransaction trans = null)
        {
            string strSql = ";with  T1 as(SELECT " +
                            ...
                            "s.[SeizureAmount],s.[ExchangeRate],s.[SeizureAmountNtd],s.[PayAmount],s.[SeizureStatus],s.[CreatedUser],s.[CreatedDate],s.[ModifiedUser],s.[ModifiedDate],s.[CancelCaseId], " +
                            "s.[TxtStatus],Convert(Nvarchar(10), m.GovDate,111) as GovDate,m.GovNo,m.GovUnit FROM [History_CaseSeizure]  s left join History_CaseMaster m on s.CaseId=m.CaseId " +
                            "WHERE s.[CancelCaseId] = @CaseId AND s.[SeizureStatus] = @SeizureStatus) ,T2 as (select ColumnValueBefore,ColumnValueAfter,ColumnID,LinkDataKey from History_CaseDataLog c1,CaseSeizure " +
        ...
        /// <returns></returns>
        public bool SaveCaseSeizureCancel(Guid caseId, List<HistoryCaseSeizure> saveList)
        {
            IDbConnection dbConnection = OpenConnection();
            IDbTransaction dbTransaction = null;
            bool rtn = true;
            ...
            {
                dbTransaction = dbConnection.BeginTransaction();
                IList<HistoryCaseSeizure> listOld = GetCaseSeizureWithCancel(caseId, dbTransaction);
                if (saveList != null && saveList.Any())
                {
                    foreach (HistoryCaseSeizure formItem in saveList)
                    {
        ...
        public bool UpdateCaseSeizureToCancel(HistoryCaseSeizure model, IDbTransaction trans = null)
        {
            string strSql = @"UPDATE [ History_CaseSeizure]
        ...
        public int CreateCase(List<HistoryCaseAccountExternal> listResult, string memo)
        {
            IDbConnection dbConnection = OpenConnection();
            IDbTransaction dbTransaction = null;
            try
            {
                using (dbConnection)
                {
                    ...
                    HistoryCaseAccountBiz cab = new HistoryCaseAccountBiz();
                    cab.DeleteCAENalByCaseID(listResult[0].CaseId, dbTransaction);
                    int a = 0;
                    foreach (HistoryCaseAccountExternal item in listResult)
                    {
        ...
        public int DeleteCAENalByCaseID(Guid caseId, IDbTransaction trans = null)
        {
            try
            {
                string strSql = @"DELETE [HistoryCaseAccountExternal] WHERE [CaseId]=@CaseId;";
                base.Parameter.Clear();
        ...
        public int InsertCAEnal(HistoryCaseAccountExternal listResult, int a, IDbTransaction trans = null)
        {
            string strSql = string.Empty;
            strSql = @"INSERT INTO HistoryCaseAccountExternal(CaseId,FirstCate,SecondCate,Description,UnitPrice,SortOrder,Quantity,Amount) values(@CaseId,@FirstCate,@SecondCate,@Description,@UnitPrice," + a + ",@Quantity,@Amount);";
            base.Parameter.Clear();
        ...
        public IList<HistoryCaseAccountExternal> GetDataFromCAEnal(Guid caseId)
        {
            string strSql = @"SELECT [CaseId],[FirstCate],[SecondCate],[Description],[UnitPrice],[SortOrder],[Quantity],[Amount] FROM [HistoryCaseAccountExternal] WHERE [CaseId]=@CaseId ORDER BY [SortOrder] ASC";
            try
            {
                Parameter.Clear();
                Parameter.Add(new CommandParameter("@CaseId", caseId));
                IList<HistoryCaseAccountExternal> list = base.SearchList<HistoryCaseAccountExternal>(strSql);
                return list;
            }
            catch (Exception ex)
            {
        ...
        public IList<HistoryCaseAccountExternal> GetDataFromCAESetting()
        {
            string strSql = @"SELECT [FirstCate],[SecondCate],[Description],[UnitPrice],[SortOrder] FROM [HistoryCaseAccountExternalSetting] ORDER BY [SortOrder] ASC";
            try
            {
                Parameter.Clear();
                IList<HistoryCaseAccountExternal> ilst = base.SearchList<HistoryCaseAccountExternal>(strSql);
                return ilst;
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetHistoryCaseAccountExternalByCaseIdList(List<string> caseIdList)
        {
            string strsql = @"SELECT [CaseId]
                                    ...
                                    ,[Quantity]
                                    ,[Amount]
                                FROM [HistoryCaseAccountExternal] AS M
                                WHERE 1=2 ";
            Parameter.Clear();
        ...
        public DataTable GetCaseReceiptByCaseIdList(List<string> caseIdList)
        {
            ...
            string strsql = @"SELECT 
	                            ...
	                            (SELECT ISNULL(SUM(Amount),0) FROM [HistoryCaseAccountExternal] AS CAE WHERE CAE.CaseId = M.CaseId)
	                            ELSE 
            ...
            string strsql = @"SELECT 
	                            ...
	                            CASE 
								WHEN M.CaseKind = '外來文案件' THEN (SELECT ISNULL(SUM(Amount),0) FROM [HistoryCaseAccountExternal] AS CAE WHERE CAE.CaseId = M.CaseId)
								ELSE 
	                            ISNULL(Fee,0)
	                             END Fee,
        ...
        /// </summary>
        /// <returns></returns>
        public string SeizureTelegram(HistoryCaseSeizure item, string memo, Guid CaseId, string CaseKind2, string BreakDay, string kind, User LogonUser)
        {
            bool rtn = false;
            ...
            if (retS.StartsWith("0000"))
            {
                ...
                log.TITLE = "經辦作業-待辦理";
                log.TXType = "發查扣押電文";
                HistoryCaseSeizure csOld = new HistoryCaseSeizure();
                if(item.SeizureId > 0)
                {
        ...
        /// </summary>
        /// <returns></returns>
        public string ResetTelegram(HistoryCaseSeizure item, string memo, Guid CaseId, string kind, User LogonUser, string BreakDay)
        {
            string newBreakday = "";
            if (BreakDay.Length > 0)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public string PayTelegram(HistoryCaseSeizure item, string memo, Guid CaseId, string BreakDay, string kind, User LogonUser)
        {
            bool rtn = false;
            string result = "發查電文后,更新表失敗";
            ExecuteHTG objHTG = new ExecuteHTG(LogonUser.Account, LogonUser.LDAPPwd, LogonUser.RCAFAccount, LogonUser.RCAFPs, LogonUser.RCAFBranch);
            ...
            if (retP.StartsWith("0000"))
            {
                ...
                HistoryCaseSeizure csOld = GetCaseSeizureInfoBySeizureId(item.SeizureId);
                if (kind == "Pay")
                {
                    if (item.PayAmount == item.SeizureAmount)//支付金額=扣押金額
                    {
...
drop table #tmp";
        ...
        /// </summary>
        public HistoryCaseSeizure GetCaseSeizureInfoBySeizureId(int seizureId)
        {
            string strSql = @"select * from  History_CaseSeizure where SeizureId=@SeizureId";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@SeizureId", seizureId));
            return SearchList<HistoryCaseSeizure>(strSql)[0];
        }
        ...
        /// </summary>
        public bool UpdateSeizureStatus(HistoryCaseSeizure item, IDbTransaction trans = null)
        {
            string strSql = @"UPDATE [ History_CaseSeizure]
                            SET 
                                [SeizureStatus]=@SeizureStatus,
