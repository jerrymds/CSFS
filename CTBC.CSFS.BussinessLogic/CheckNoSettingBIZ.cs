...
{
    public class CheckNoSettingBIZ : CommonBIZ
    {
        public CheckNoSettingBIZ(AppController appController)
            : base(appController)
        { }

        public CheckNoSettingBIZ()
        { }
        ...
        /// <returns></returns>
        public IList<CheckNoSetting> GetQueryList(CheckNoSetting cns, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                ...
                sqlStr += @";with T1 
	                        ...
	                        (
                               ...
                               ,[WeekTempAmount]
                               ,[UseStatus]
                               FROM [CheckInterval] where 1=1" + sqlWhere + @"
	                        ),T2 as
	                        (
                ...
                IList<CheckNoSetting> _ilsit = base.SearchList<CheckNoSetting>(sqlStr);
                ...
                {
                    base.DataRecords = 0;
                    _ilsit = new List<CheckNoSetting>();
                }
                return _ilsit;
            }
            catch (Exception ex)
        ...
        /// <returns></returns>
        public int Create(CheckNoSetting model)
        {
            int rtn = 0;
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    string sqlStr = @"DECLARE @ID BIGINT; 
                                      SELECT @ID=ISNULL(MAX(CheckIntervalID),0) +1 FROM CheckInterval;
                                      INSERT INTO CheckInterval
        ...
        /// <returns></returns>
        public CheckNoSetting Select(Int64 CheckIntervalID)
        {
            try
            {
                string sqlStr = @"SELECT [CheckIntervalID]
                               ...
                               ,[WeekTempAmount]
                               ,[UseStatus] 
                                from CheckInterval where CheckIntervalID=@CheckIntervalID";

                base.Parameter.Clear();
                base.Parameter.Add(new CommandParameter("@CheckIntervalID", CheckIntervalID));

                IList<CheckNoSetting> list = base.SearchList<CheckNoSetting>(sqlStr);
                if (list != null)
                {
                    if (list.Count > 0)
                    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int Edit(CheckNoSetting model)
        {
            int rtn = 0;
            IDbConnection dbConnection = base.OpenConnection();
            IDbTransaction dbTransaction = null;
            try
            {
                ...
                {
                    dbTransaction = dbConnection.BeginTransaction();
                    string sqlStr = @"update CheckInterval set 
        ...
        public int Delete(Int64 CheckIntervalID)
        {
            ...
            try
            {
                ...
                {
                    dbTransaction = dbConnection.BeginTransaction();
                    string sqlStr = @"delete from CheckInterval where CheckIntervalID=@CheckIntervalID";
        ...
        public int Active(CheckNoSetting model)
        {
            ...
            try
            {
                ...
                {
                    dbTransaction = dbConnection.BeginTransaction();
                    string sqlStr = @"update CheckInterval set UseStatus=@UseStatus where CheckIntervalID=@CheckIntervalID;";
                    base.Parameter.Clear();
                    base.Parameter.Add(new CommandParameter("@CheckIntervalID", model.CheckIntervalID));
                    base.Parameter.Add(new CommandParameter("@UseStatus", model.UseStatus));
                    ...
                    {
                        sqlStr = @"INSERT INTO [CheckUse] ([CheckIntervalID],[CheckNo],[Kind],[IsUsed],[IsPreserve])VALUES (@CheckIntervalID,@CheckNo,@Kind,@IsUsed,@IsPreserve);";
        ...
        /// <param name="strSortDirection"></param>
        /// <returns></returns>
        public IList<CheckNoSetting> GetDetailList(Int64 checkIntervalId, Int32? checkNoS, Int32? checkNoE, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                base.PageIndex = pageIndex;
                ...
                sqlStr += @";with T1 
	                        ...
	                        (
                               ...
                               ,[IsUsed]
                               ,[IsPreserve]
                               FROM [CheckUse] where 1=1" + sqlWhere + @"
	                        ),T2 as
	                        (
		                        select *, row_number() over (order by " + strSortExpression + " " + strSortDirection + @" ) RowNum
		                        from T1
        ...
        /// </summary>
        /// <returns></returns>
        public CheckUse GetNoUseCheck(IDbTransaction trans = null)
        {
            string sql = @"SELECT 
	                            D.[CheckIntervalID],
	                            D.[CheckNo],
            ...
            Parameter.Clear();
            IList<CheckUse> list = trans == null ? SearchList<CheckUse>(sql) : SearchList<CheckUse>(sql, trans);
        ...
        /// </summary>
        /// <returns></returns>
        public CheckUse GetPreserveCheck(IDbTransaction trans = null)
        {
            string sql = @"SELECT 
	                            D.[CheckIntervalID],
	                            D.[CheckNo],
        ...
        /// <returns></returns>
        public JsonReturn UseCheck(ref CheckUse check, IDbTransaction trans = null)
        {
            //* 沒有支票號碼取一個支票號碼
            if (check == null || check == new CheckUse())
            {
            ...
            return SettingForUseCheck(new CheckNoSetting { Kind = CheckNoUseKind.CasePay, IsUsed = 1, CheckNo = check.CheckNo, CheckIntervalID = check.CheckIntervalId }, trans)
        ...
        public JsonReturn CancelCheck(string checkIntervalId, string checkNo, string sendId, IDbTransaction trans = null)
        {
            ...
            {
                if (!Setting(new CheckNoSetting { Kind = CheckNoUseKind.CasePay, IsUsed = 0, CheckNo = Convert.ToInt64(checkNo), CheckIntervalID = Convert.ToInt64(checkIntervalId) }, trans))
        ...
        public JsonReturn DisableCheck(string checkIntervalId, string checkNo, string sendId, IDbTransaction trans = null)
        {
            ...
            if (!string.IsNullOrEmpty(checkIntervalId) && !string.IsNullOrEmpty(checkNo))
            {
                if (!Setting(new CheckNoSetting { Kind = "作廢", IsUsed = 1, CheckNo = Convert.ToInt64(checkNo), CheckIntervalID = Convert.ToInt64(checkIntervalId) }, trans))
                    return new JsonReturn { ReturnCode = "0", ReturnMsg = Lang.csfs_cancelticket + Lang.csfs_fail };
        ...
        {
            CasePayeeSetting old = new CasePayeeSettingBIZ().Select(payeeId);
            if (old == null)
            ...
            try
            {
                using (dbConnection)
                {
                    dbTransaction = dbConnection.BeginTransaction();
                    //* 取號
                    CheckUse check = GetPreserveCheck(dbTransaction);
                    //* 沒有新支票號了
                    if (check == null)
                    {
                        dbTransaction.Rollback();
                    ...
                    CaseSendSettingBIZ sendBiz = new CaseSendSettingBIZ();
                    //* 取得初始的發文資訊資料
                    CaseSendSettingCreateViewModel caseSendModel = sendBiz.GetDefaultSendSetting(old, out errMsg, dbTransaction);
        ...
        /// <returns></returns>
        public bool SettingForUseCheck(CheckNoSetting cns, IDbTransaction trans = null)
        {
            string sqlStr = @"UPDATE CheckUse SET Kind=@Kind,IsUsed=@IsUsed WHERE CheckNo=@CheckNo AND CheckIntervalID = @CheckIntervalID and IsUsed=0";
            Parameter.Clear();
        ...
        /// <param name="trans"></param>
        /// <returns></returns>
        public bool Setting(CheckNoSetting cns, IDbTransaction trans = null)
        {
            string sqlStr = @"UPDATE CheckUse SET Kind=@Kind,IsUsed=@IsUsed WHERE CheckNo=@CheckNo AND CheckIntervalID = @CheckIntervalID";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("CheckNo", cns.CheckNo));
            Parameter.Add(new CommandParameter("CheckIntervalID", cns.CheckIntervalID));
            Parameter.Add(new CommandParameter("Kind", cns.Kind));
        ...
        {
            string strSql = "SELECT ISNULL(COUNT(1),0) FROM [CheckUse] WHERE IsUsed = 0 AND IsPreserve = 0 AND Kind = '支付'";
            Parameter.Clear();
        ...
        {
            string strSql = @"UPDATE [CheckUse] SET IsPreserve = 1 
                                WHERE [CheckNo] IN 
                                (
	                                SELECT TOP " + num + @" [CheckNo] FROM [CheckUse]
        ...
        {
            string strSql = @"UPDATE [CheckUse] SET IsPreserve = 0 
        ...
        {
            string strSql =
                @"SELECT ISNULL(COUNT(1),0) FROM [CheckUse] WHERE IsUsed = 0 AND IsPreserve = 1 AND Kind = '支付'";
            Parameter.Clear();
            int? tmp = ExecuteScalar(strSql) as int?;
            return tmp ?? 0;
        }
