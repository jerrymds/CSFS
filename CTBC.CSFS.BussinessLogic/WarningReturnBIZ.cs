...
{
    public class WarningReturnBIZ : CommonBIZ
    {
        public WarningReturnBIZ(AppController appController)
            : base(appController)
        { }

        public WarningReturnBIZ()
        { }

        public IList<WarningReturn> GetQueryList(WarningReturn wq)
        {
            try
            {
                string sqlStr = "";
                ...
                if (!string.IsNullOrEmpty(wq.Set))
                {
                    if (wq.Set == "Y")
                    {
                        sqlStrWhere += @" AND ( [Set] = '1' or Flag_909113 = '1')  ";
                    }
                    else
                    {
...
case ISNULL(ws.NoClosed,0)
 ...
 //               WHERE wd.Status = 'C01'  " + sqlStrWhere + @" ) 
	//select* from t2 ORDER BY DocNo ASC,ForCDate ASC ";
                IList<WarningReturn> _ilsit = base.SearchList<WarningReturn>(sqlStr);
        ...
        public DataTable ExcelList(WarningReturn wq)
        {
            string sqlStrWhere = "";
            if (!string.IsNullOrEmpty(wq.CustId))
            {
        ...
        //public MemoryStream ParmCodeExcel_NPOI(WarningReturn wq)
        //{
  ...
  on wm.DocNo like '%'+wg.CHQ_PAYEE+'%' where wm.docno = @DocNo ";
        ...
        public MemoryStream ParmCodeExcel_NPOI(WarningReturn model)
        {
            ...
            IList<WarningReturn> ilst = GetQueryList(model);
            ...
            if (ilst != null)
            {
                ms = ExcelExport(ilst, headerColumns,
                                                   delegate (HSSFRow dataRow, WarningReturn dataItem)
                                                   {
