...
{
    public class HistoryLendDataBIZ : CommonBIZ
    {
        public HistoryLendDataBIZ(AppController appController)
            : base(appController)
        { }

        public HistoryLendDataBIZ()
        { }
        ...
        /// <returns></returns>
        public bool CreateLend(HistoryAgentOriginalInfoViewModel model)
        {
            HistoryCaseMasterBIZ caseBiz = new HistoryCaseMasterBIZ();
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    string lendid = "0";
                    #region 主表
                    HistoryLendDataBIZ lenddata = new HistoryLendDataBIZ();
                    HistoryCaseMaster casemodel = caseBiz.MasterModel(model.LendDataInfo.CaseId);
                    ...
                    #region 附件
                    HistoryLendAttachmentBIZ lendAttach = new HistoryLendAttachmentBIZ();
                    foreach (HistoryLendAttachment attach in model.LendAttachmentInfoList)
                    {
        ...
        /// <returns></returns>
        public int Create(HistoryLendData model, ref string lendid, IDbTransaction trans = null)
        {
            string sql = @"insert into History_LendData  (CaseId,DocNo,ClientID,Name,BankID,Bank,Account,Memo,Phone,LendStatus) 
                                        values (
        ...
        /// <returns></returns>
        public IList<HistoryLendData> GetQueryList(HistoryAgentOriginalInfoViewModel model, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                ...
                */
                sqlStr = @" select ld.[LendID],ld.[CaseId],ld.[DocNo],ld.[ClientID],ld.[Name],
                                  ld.[BankID],ld.[Bank],ld.[Account],ld.[Memo],ld.[Phone]
                                 from History_LendData  ld where  ld.CaseId=@CaseId ";

                IList<HistoryLendData> _ilsit = base.SearchList<HistoryLendData>(sqlStr);

                if (_ilsit != null && _ilsit.Count > 0)
                {
                    string strID = string.Empty;
                    foreach (HistoryLendData item in _ilsit)
                    {
                        strID+="'"+item.CaseId+"',";
                    }
                    strID = strID.TrimEnd(',');
                    string strName = string.Empty;
                    string sql = "SELECT LendAttachName,CaseId FROM History_LendAttachment WHERE CaseId In (" + strID + ")";
                    List<HistoryLendAttachment> listAttach = base.SearchList<HistoryLendAttachment>(sql).ToList();
                    if (listAttach != null && listAttach.Any())
                    {
                        foreach (HistoryLendData item in _ilsit)
                        {
                            foreach (HistoryLendAttachment items in listAttach.Where(m=>m.CaseId==item.CaseId))
                            {
                ...
                {
                    return new List<HistoryLendData>();
                }
            }
            catch (Exception ex)
            {
        ...
        public HistoryLendData getModel(string lendId)
        {
            ...
            IList<HistoryLendData> _list = base.SearchList<HistoryLendData>(strSql);
        ...
        {
            string strSql = " delete from History_LendAttachment where LendAttachId=@LendAttachId ";
            base.Parameter.Clear();
        ...
        {
            string strSql = @" delete from  History_LendData where LendID=@LendID";
            Parameter.Clear();
            // 添加參數
            Parameter.Add(new CommandParameter("@LendID", lendId));
        ...
        public bool UpdateLend(HistoryAgentOriginalInfoViewModel model)
        {
            IDbConnection dbConnection = OpenConnection();
            IDbTransaction dbTransaction = null;
            try
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int Update(HistoryLendData model, IDbTransaction trans = null)
        {
            string strSql = @" update History_LendData set ClientID=@ClientID,Name=@Name,BankID=@BankID,Bank=@Bank,
        ...
        /// <param name="strSortDirection"></param>
        /// <returns></returns>
        public IList<HistoryLendData> GetLendCaseQueryList(HistoryAgentOriginalInfoViewModel model, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                base.PageIndex = pageIndex;
                ...
                sqlStr = @"select ld.[LendID],ld.[CaseId],ld.[DocNo],ld.[ClientID],ld.[Name],M.[CaseNo],ld.[BankID],ld.[Bank],
                              ...
                              from History_LendData ld LEFT OUTER JOIN History_CaseMaster AS M ON LD.CaseId = M.CaseId
        ...
        /// <param name="strSortDirection"></param>
        /// <returns></returns>
        public IList<HistoryLendData> GetLendQueryList(HistoryAgentOriginalInfoViewModel model, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                base.PageIndex = pageIndex;
                ...
                sqlStr = @" select ld.[LendID],ld.[CaseId],ld.[DocNo],ld.[ClientID],ld.[Name],cm.[CaseNo],ld.[BankID],ld.[Bank],
                              ...
                              CONVERT(varchar(100),ld.[ReturnBankDate],111) as [ReturnBankDate],ld.[ReturnPostNo],ld.[BankReceiver],ld.[ReturnMemo]  
                              ,cm.GovNo
                              from History_LendData  ld left join History_CaseMaster cm on ld.CaseId=cm.CaseId
                              where 1=1 " + sqlWhere;
        ...
        /// <param name="lendId"></param>
        /// <returns></returns>
        public HistoryLendData getLendModel(string lendId)
        {
            string strSql = @"select ld.[LendID],ld.[CaseId],ld.[DocNo],m.[CaseNo],ld.[ClientID],ld.[Name],
                                       ...
                                       CONVERT(varchar(100),ld.[ReturnBankDate],111) as [ReturnBankDate],ld.[ReturnPostNo],ld.[BankReceiver],ld.[ReturnMemo]  
                                       from History_LendData  ld left join History_CaseMaster m on ld.CaseId=m.CaseId where ld.LendID=@LendID";
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int UpdateLend(HistoryLendData model)
        {
            string strSql = @" update History_LendData set DocNo=@DocNo,ClientID=@ClientID,Name=@Name,Account=@Account,
        ...
        {
            string strSql = @" update  History_LendData set LendStatus=@LendStatus,ReturnDate=null,ReturnCaseId=null,ReturnBankDate=null,RetrunBankID=null,
        ...
        #region 正本備查查詢送件
        public IList<HistoryLendData> LendDataSearchList(HistoryLendData model, int pageIndex, string strSortExpression, string strSortDirection)
        {
            try
            {
                base.PageIndex = pageIndex;
                ...
                sqlStr += @";with T1 
		                    ...
		                    select ROW_NUMBER() OVER(ORDER BY LendId asc) as Sno,l.[LendID],l.[CaseId],l.[DocNo],l.[ClientID],l.[Name],l.[BankID],l.[Bank],l.[Account],l.[Memo]
                                    ...
                                    ,l.[ReturnMemo],l.[LendStatus],(select CaseNo from History_CaseMaster m where m.CaseId=l.[ReturnCaseId]) AS returnCaseNo,c.[Status],c.[CaseNo],c.[GovKind],c.[GovUnit]
                                    ,c.[GovDate],c.[Speed],c.[ReceiveKind],c.[GovNo],c.[CaseKind],c.[CaseKind2],c.[AgentUser]
                                    ,(SELECT TOP 1 CONVERT(varchar(12),m.SendDate,111) FROM History_CaseSendSetting m where m.CaseId = l.CaseId ORDER BY SendDate desc) AS SendDate,
                                    (SELECT TOP 1 SendNo FROM History_CaseSendSetting m where m.CaseId = l.CaseId ORDER BY SendDate desc) AS SendNo,
                                    (select MAX( MailNo) from History_MailInfo mi where l.CaseId=mi.CaseId) as MailNo,
                                    (select top 1 CONVERT(nvarchar(10), mi.MailDate,111) from History_MailInfo mi where l.CaseId=mi.CaseId order by MailNo) as MailDate
                                    from History_LendData l left join History_CaseMaster c on l.CaseId=c.CaseId 
                                    where 1=1  " + sqlWhere + @"   
	                        ),T2 as
	                        (
                ...
                if (_ilsit != null)
                {
                    ...
                    {
                        base.DataRecords = 0;
                        _ilsit = new List<HistoryLendData>();
                    }
                    return _ilsit;
                }
                else
        ...
        {
            string strSql = "DELETE History_LendData WHERE  CaseId = @CaseId ";
            Parameter.Clear();
        ...
        {
            string strSql = "DELETE History_LendAttachment WHERE  CaseId = @CaseId ";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("CaseId", caseId));
            return dbtrans == null ? ExecuteNonQuery(strSql) > 0 : ExecuteNonQuery(strSql, dbtrans) > 0;
        }
        ...
        {
            string strSql = @" update History_LendData set ReturnCaseId=NULL  where ReturnCaseId=@ReturnCaseId";
            Parameter.Clear();
