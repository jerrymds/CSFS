...
namespace CTBC.CSFS.BussinessLogic
{
    public class AgentSettingBIZ : CommonBIZ
    {
        ...
        public IList<AgentSetting> getAgentSetting(int IsSeizure = 1)
        {
            try
            {
                string strSql = @"SELECT * FROM AgentSetting  WHERE IsSeizure=@IsSeizure";
                base.Parameter.Clear();
                base.Parameter.Add(new CommandParameter("@IsSeizure", IsSeizure));
                return base.SearchList<AgentSetting>(strSql);
            }
        ...
        //查詢扣押經辦人員
        private string GetAgentSettingIsSeizure()
        {
            try
            {
                string strSql = @"select EmpId as AsempId from AgentSetting where IsSeizure =1";
                IList<AgentSetting> list = base.SearchList<AgentSetting>(strSql);
                if (list.Count > 0)
                {
                    string Empid = string.Empty;
        ...
        //查詢外來文經辦人員
        private string GetAgentSettingIsCase()
        {
            try
            {
                string strSql = @"select EmpId as AsempId from AgentSetting where IsCase =1";
                IList<AgentSetting> list = base.SearchList<AgentSetting>(strSql);
                if (list.Count > 0)
                {
                    string Empid = string.Empty;
        ...
        public string Dosave(string Empidarr, string Department, string IsAutoDispatch, string IsAutoDispatchFS)
        {
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    string[] SectionEmpId = GetAgentSetting().Split(',');//查找AgentSetting中所有人員
                    string[] IsSeizureEmpId = GetAgentSettingIsSeizure().Split(',');//查找AgentSetting中扣押經辦人員
                    string[] IsCaseEmpId = GetAgentSettingIsCase().Split(',');//查找AgentSetting中外來文經辦人員
                    List<string> InsertEmpId = new List<string>();//要insert的資料
                    List<string> DeleteEmpId = new List<string>();//要刪除的資料
                    List<string> UpdateIsSeizureEmpId = new List<string>();//要更新的扣押經辦
                    List<string> UpdateIsCaseEmpId = new List<string>();//要更新的外來文經辦
                    if (SectionEmpId.Count() > 0 && !string.IsNullOrEmpty(SectionEmpId[0]))
                    {
                        foreach (string item in SectionEmpId)
                        {
                    ...
                    foreach (string id in DeleteEmpId)//刪除的授權人員
                    {
                        string Dsql = @"delete  from AgentSetting where EmpId=@EmpId";
                        base.Parameter.Clear();
                        base.Parameter.Add(new CommandParameter("@EmpId", id));
                        rtn = base.ExecuteNonQuery(Dsql, dbTransaction);
                    }
                    if (Empid.Count() > 0 && !string.IsNullOrEmpty(Empid[0]))
                    {
                        foreach (string item in Empid)
                        {
                            if (!SectionEmpId.Contains(item))
                            {
                                InsertEmpId.Add(item);
                            }
                        }
                    }
                    foreach (string id in InsertEmpId)//新增的授權人員
                    {
                        //20170421 固定 RQ-2015-019666-017 派件至跨單位 宏祥 update start
                        ...
                        //20170421 固定 RQ-2015-019666-017 派件至跨單位 宏祥 update start
                        base.Parameter.Add(new CommandParameter("@IsSeizure", "0"));//0否,1是,新增都為0,之後再更新
                        base.Parameter.Add(new CommandParameter("@IsCase", "0"));//0否,1是,新增都為0,之後再更新
                        rtn = base.ExecuteNonQuery(sqlStr, dbTransaction);
                    }
                    if (IsSeizureEmpId.Count() > 0 && !string.IsNullOrEmpty(IsSeizureEmpId[0]))
                    {
                        foreach (string item in IsSeizureEmpId)
                        {
                            if (!EmpSeizure.Contains(item))
                            {
                                UpdateIsSeizureEmpId.Add(item);
                            }
                        }
                    }
                    if (EmpSeizure.Count() > 0 && !string.IsNullOrEmpty(EmpSeizure[0]))
                    {
                        foreach (string item in EmpSeizure)
                        {
                            if (!IsSeizureEmpId.Contains(item))
                            {
                                UpdateIsSeizureEmpId.Add(item);
                            }
                        }
                    }
                    foreach (string item in UpdateIsSeizureEmpId)//更新扣押經辦
                    {
                        string Ssql = @"select * from AgentSetting where EmpId=@EmpId";
                        base.Parameter.Clear();
                        base.Parameter.Add(new CommandParameter("@EmpId", item));
                        AgentSetting agentSetting = base.SearchList<AgentSetting>(Ssql, dbTransaction)[0];
                        string isSeizure = "";
                        if (agentSetting != null && agentSetting.IsSeizure)
                        {
                            isSeizure = "0";
                        }
                        else
                        ...
                        string UpdateSql = @"update AgentSetting set IsSeizure = @IsSeizure where SettingId = @SettingId";
                        base.Parameter.Clear();
                        base.Parameter.Add(new CommandParameter("@SettingId", agentSetting.SettingId));
                        base.Parameter.Add(new CommandParameter("@IsSeizure", isSeizure));//0否,1是
                        rtn = base.ExecuteNonQuery(UpdateSql, dbTransaction);
                    }
                    if (IsCaseEmpId.Count() > 0 && !string.IsNullOrEmpty(IsCaseEmpId[0]))
                    {
                        foreach (string item in IsCaseEmpId)
                        {
                            if (!EmpCase.Contains(item))
                            {
                                UpdateIsCaseEmpId.Add(item);
                            }
                        }
                    }
                    if (EmpCase.Count() > 0 && !string.IsNullOrEmpty(EmpCase[0]))
                    {
                        foreach (string item in EmpCase)
                        {
                            if (!IsCaseEmpId.Contains(item))
                            {
                                UpdateIsCaseEmpId.Add(item);
                            }
                        }
                    }
                    foreach (string item in UpdateIsCaseEmpId)//更新外來文經辦
                    {
                        string Ssql = @"select * from AgentSetting where EmpId=@EmpId";
                        base.Parameter.Clear();
                        base.Parameter.Add(new CommandParameter("@EmpId", item));
                        AgentSetting agentSetting = base.SearchList<AgentSetting>(Ssql, dbTransaction)[0];
                        string isCase = "";
                        if (agentSetting != null && agentSetting.IsCase)
                        {
                            isCase = "0";
                        }
                        else
                        ...
                        string UpdateSql = @"update AgentSetting set IsCase = @IsCase where SettingId = @SettingId";
                        base.Parameter.Clear();
                        base.Parameter.Add(new CommandParameter("@SettingId", agentSetting.SettingId));
