...
namespace CTBC.CSFS.BussinessLogic
{
    public class WarningQueryBIZ : CommonBIZ
    {
        ...
        public IList<WarningQuery> GetQueryList(WarningQuery wq)
        {
            try
            {
                ...
                {
                    sqlStrWhere += @" AND wd.No_165 like @No_165";
                ...
                {
                    sqlStrWhere += @" AND convert(varchar, RelieveDate, 111) >= @RelieveDateS";
                    base.Parameter.Add(new CommandParameter("@RelieveDateS", wq.RelieveDateS));
                }
                if (!string.IsNullOrEmpty(wq.RelieveDateE))
                {
                    sqlStrWhere += @" AND convert(varchar, RelieveDate, 111) <= @RelieveDateE";
                ...
                {
                    sqlStrWhere += @" AND (convert(varchar, wd.ModifiedDate, 111) >= @ModifyDateS or convert(varchar, wm.ModifiedDate, 111) >= @ModifyDateS )";
                    base.Parameter.Add(new CommandParameter("@ModifyDateS", wq.ModifyDateS));
                }
                if (!string.IsNullOrEmpty(wq.ModifyDateE))
                {
                    sqlStrWhere += @" AND (convert(varchar, wd.ModifiedDate, 111) <= @ModifyDateE or convert(varchar, wm.ModifiedDate, 111) <= @ModifyDateE )";
    ...
    Convert(Nvarchar(10),ForCDate,111 ) as ForCDate, NotificationUnit,CustName,ExtPhone,wd.NotificationSource,NotificationName,PoliceStation,
	 ...
	 AS NoClosed,HappenDateTime,wd.No_165,DocAddress,UniteNo,Retry,Kind,
    case StateType 
    when  '1' then '新增' 
    when '4' then '修改'
    when '5' then '撤銷'
            ...
            if (ilst != null)
            {
                ms = ExcelExport(ilst, headerColumns,
                                                   delegate (HSSFRow dataRow, WarningQuery dataItem)
                                                   {
                                                       ...
                                                       dataRow.CreateCell(0).SetCellValue(dataItem.Kind);
                                                       ...
                                                       dataRow.CreateCell(11).SetCellValue(dataItem.NotificationName);
                                                       dataRow.CreateCell(12).SetCellValue(dataItem.CustName);
                                                       dataRow.CreateCell(13).SetCellValue(dataItem.PoliceStation);
                                                       dataRow.CreateCell(14).SetCellValue(dataItem.AccountStatusName);
                                                       dataRow.CreateCell(15).SetCellValue(dataItem.NoClosedName);//結案
                                                       dataRow.CreateCell(16).SetCellValue(dataItem.currency);//幣別
                                                       dataRow.CreateCell(17).SetCellValue(dataItem.NotifyBal);
                                                       dataRow.CreateCell(18).SetCellValue(dataItem.ReleaseBal);
                                                       dataRow.CreateCell(19).SetCellValue(dataItem.HappenDateTime);//案發時間
                                                       dataRow.CreateCell(20).SetCellValue(dataItem.DocAddress);//地點
                                                       dataRow.CreateCell(21).SetCellValue(dataItem.No_165);//165案號
                                                       if (!string.IsNullOrEmpty(dataItem.Retry))                                {
                                                           if (dataItem.Retry == "Y")
                                                           {
                                                               dataRow.CreateCell(22).SetCellValue("Y");
                                                           }
                                                           else
                                                           {
                                                               dataRow.CreateCell(22).SetCellValue("N");
                                                           }
                                                       }
                                                       else
                                                       {
                                                       ...
                                                       dataRow.CreateCell(23).SetCellValue(dataItem.UniteNo);//165案號
                                                   });
            }
            return ms;
        }
