...
namespace CTBC.CSFS.BussinessLogic
{
    public class SendEDocBiz : CommonBIZ
    {
        ...
        public bool InsertCaseMasterDoc(Guid caseId,string AgentUser,string Address,string Tel,string Fax,string Title,string MasterName)
        {
            try
            {
                string sql = @"
                    Delete CaseMasterDoc where CaseId = @CaseId;
                    INSERT INTO [dbo].[CaseMasterDoc]
                           ([CaseId]
                           ,[AgentUser]
                           ,[Address]
                           ,[Tel]
                ...
                Parameter.Add(new CommandParameter("@AgentUser", AgentUser));
                Parameter.Add(new CommandParameter("@Address", Address));
                Parameter.Add(new CommandParameter("@Tel", Tel));
                Parameter.Add(new CommandParameter("@Fax", Fax));
                Parameter.Add(new CommandParameter("@Title", Title));
                Parameter.Add(new CommandParameter("@MasterName", MasterName));
                return (int)base.ExecuteNonQuery(sql) > 0;  
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetGssDocCaseID(string SendBatchNo)
        {
            DataTable dt = new DataTable();
            try
            {                
                string sql = @"Select Caseid from GssDoc_Detail where SendBatchNo='" + SendBatchNo + "' ";
                dt = base.Search(sql) ;
            }
            catch (Exception ex)
            {
        ...
        public bool UpdateGssDoc(string SendBatchNo)
        {
            try
            {
                string sql = @"update GssDoc_Detail set SendStatus=3 where SendBatchNo='" + SendBatchNo + "' ";
                return (int)base.ExecuteNonQuery(sql) > 0;
            }
            catch (Exception ex)
            {
