...
{
    public class DirectorToApproveBIZ : CommonBIZ
    {
        private static List<Guid> staticAryCaseId = new List<Guid>();
        public DirectorToApproveBIZ(AppController appController)
            : base(appController)
        { }

        public DirectorToApproveBIZ()
        { }
        /// <summary>
        /// 取得主管核決的結果集
        /// </summary>
        ...
        public IList<DirectorToApprove> GetDirectorToApproveData(DirectorToApprove model, int pageIndex, string strSortExpression, string strSortDirection, string EmpId)
        {
            try
            {
                ...
                {
                   sqlWhere += @" AND AgentUser in (select EmpID from V_AgentAndDept  WITH (NOLOCK) where ManagerID like @ManagerID) ";
                ...
                if (!string.IsNullOrEmpty(model.SendDateS) || !string.IsNullOrEmpty(model.SendDateE))
                {
                    ...
                    sqlWhere += " AND B.CaseId IN (SELECT CaseId  FROM CaseSendSetting AS CSS  WITH (NOLOCK) where 1=1 " + sqlWhere1 + @") ";
                }
                ...
                //sqlWhere += @" and B.CaseId in (select CaseId from CaseHistory ch1 where ToRole like '自動%退回人工' and convert(varchar(10),CreatedDate,23) = (select convert(varchar(10),max(CreatedDate),23) from CaseHistory ch2 where  ch1.CaseId = ch2.CaseId)) ";
                sqlWhere += @"  and B.CaseId not in (select CaseId from CaseHistory ch1  WITH (NOLOCK)  where FromRole like '自動%' and FromRole <> '自動扣押退回人工' and  CreatedDate = (select max(CreatedDate) from CaseHistory ch2  WITH (NOLOCK) where  ch1.CaseId = ch2.CaseId)) ";
                string strSql = @";with T1 
	                        as
	                        (
		                       SELECT distinct B.Status,B.CaseId,B.CaseNo,B.GovUnit,(SELECT CONVERT(varchar(100), B.GovDate, 111)) as GovDate,B.GovNo,B.Person,B.AgentUser,B.CaseKind,B.CaseKind2,B.Speed, (SELECT TOP 1 CONVERT(varchar(12),m.SendDate,111) FROM CaseSendSetting m  WITH (NOLOCK) where m.CaseId = B.CaseId ORDER BY CreatedDate desc) AS SendDate,(SELECT CONVERT(varchar(100), B.LimitDate, 111)) as LimitDate FROM CaseAssignTable AS A  WITH (NOLOCK) 
                                LEFT OUTER JOIN CaseMaster AS B  WITH (NOLOCK) ON A.CaseId=B.CaseId left join CaseSendSetting CSS  WITH (NOLOCK) on CSS.CaseId=B.CaseId
	                        ...
	                        (
		                        select *,(select max(RowNum) from T2  WITH (NOLOCK) ) maxnum from T2  WITH (NOLOCK) 
		                        where rownum between @pageS and @pageE
	                        )
	                        select a.* from T3 a  WITH (NOLOCK) order by a.RowNum";
        ...
        public IList<DirectorCooperative> GetDirectorCooperativeData(DirectorCooperative model, int pageIndex, string strSortExpression, string strSortDirection, string EmpId)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.SendDateS) || !string.IsNullOrEmpty(model.SendDateE))
                {
                    ...
                    sqlWhere += " AND cm.CaseId IN (SELECT CaseId  FROM CaseSendSetting AS CSS  WITH (NOLOCK) where 1=1 " + sqlWhere1 + @") ";
                }
                if (!string.IsNullOrEmpty(model.Unit))
                {
                    sqlWhere += @" and Unit like @Unit ";
                ...
                sqlWhere += @" and cm.CaseId not in (select CaseId from CaseHistory ch1  WITH (NOLOCK) where FromRole like '自動%' and FromRole <> '自動扣押退回人工' and CreatedDate = (select max(CreatedDate) from CaseHistory ch2  WITH (NOLOCK) where  ch1.CaseId = ch2.CaseId)) ";
                string strSql = @";with T1 
	                        as
	                        (
                               SELECT distinct cm.Status,cm.CaseId,cm.CaseNo,cm.GovUnit,(SELECT CONVERT(varchar(100), cm.GovDate, 111)) as GovDate,
                                (SELECT TOP 1 CONVERT(varchar(12),m.SendDate,111) FROM CaseSendSetting m  WITH (NOLOCK) where m.CaseId = cm.CaseId ORDER BY cm.CreatedDate desc) AS SendDate,
                               cm.GovNo,cm.Person,cm.AgentUser,cm.CaseKind,cm.CaseKind2,cm.Speed,(SELECT CONVERT(varchar(100), cm.LimitDate, 111)) as LimitDate
                                FROM CaseMaster cm  WITH (NOLOCK) left join CaseSendSetting css  WITH (NOLOCK) on cm.caseId=css.caseId where (cm.Status=@Status1 OR cm.Status=@Status2) AND cm.isDelete='0' " + sqlWhere + @" 
	                        ),T2 as
	                        (
		                        select *, row_number() over (order by LimitDate ASC , CaseNo ASC ) RowNum
		                        from T1  WITH (NOLOCK) 
        ...
        public IList<CaseEdocFile> GetBatchControlNotUp(string caseIdarr)
        {
            try
            {
                string sql = @"select * from CaseEdocFile  WITH (NOLOCK) where Type='發文' and caseId in (select caseId from BatchControl  WITH (NOLOCK) where STATUS_Create=1 and STATUS_Transfer=0 and caseId in (" + caseIdarr + "))";
        ...
        public bool UpdateBatchControlUp(List<Guid> aryCaseId, string userId)
        {
            ...
            catch (Exception x)
            {
                ...
                ApLog.Message = "更新交換狀態失敗:" + x.ToString();
                ApLog.dic["UserId"] = "System";
                ApLog.dic["FunctionId"] = "DirectToApproveBiz";
                _csfsLogBIZ.LogonLogoutLog(ApLog);
                // adam end
                try
                {
                ...
                using (dbConnection)
                {
                    ...
                    foreach (Guid caseId in aryCaseId)
                    {
                        ...
                        else
                        {
                            ...
                            string sql = @"if not exists (select CaseId from BatchControl  WITH (NOLOCK) where CaseId = @CaseId) Insert into BatchControl(CaseId,CREATE_TIME) values(@CaseId,getdate())"; 
                            Parameter.Clear();
                            Parameter.Add(new CommandParameter("@CaseId", caseId));
                    ...
                    foreach (Guid caseId in aryCaseId)
                    {
                        var sql = @"delete from CaseHistory  WITH (NOLOCK) where CaseId = @CaseId and Event = @Event and CreatedDate between DATEADD( minute,-30,GETDATE()) and getdate();";
                        Parameter.Clear();
                        Parameter.Add(new CommandParameter("@CaseId", caseId));
                ...
                using (dbConnection)
                {
                    ...
                    foreach (Guid caseId in aryCaseId)
                    {
                        ...
                        if (masterobj.Status == aryStatus[num])
                        {
                            ...
                            if (masterobj.AfterSeizureApproved > 0 && (mLastHistory == "主管放行結案"||mLastHistory == "主管整批放行結案"))
                ...
                string strSql = @";with T1 
	                        ...
	                        (
		                       SELECT distinct CM.CaseId,CM.CaseNo,CM.GovUnit,(SELECT CONVERT(varchar(100), CM.GovDate, 111)) as GovDate,CM.GovNo,CM.Person,CM.AgentUser,CM.CaseKind,CM.CaseKind2,CSS.Speed, (SELECT TOP 1 CONVERT(varchar(12),SendDate,111) FROM CaseSendSetting where CaseId = CM.CaseId ORDER BY CreatedDate desc) AS SendDate,(SELECT CONVERT(varchar(100), CM.LimitDate, 111)) as LimitDate FROM CaseMaster AS CM
                                ...
                                WHERE CSS.SendKind='電子發文' and CM.CaseId in (select CaseId from BatchControl  WITH (NOLOCK) where STATUS_Transfer=0 and STATUS_Create=1) " + sqlWhere + @" 
	                        ),T2 as
	                        (
		                        select *, row_number() over (order by LimitDate ASC , CaseNo ASC ) RowNum
		                        from T1  WITH (NOLOCK) 
	                        ...
	                        (
		                       SELECT DISTINCT
                                       ...
                                       --20170714 固定 RQ-2015-019666-019 派件至跨單位(廠商bug一併修正) 宏祥 add end--
                                        ...
                                        --(select top 1 MailNo from MailInfo  WITH (NOLOCK)  where CaseId=A.CaseId) as MailNo,
										M.MailNo,
										convert(varchar(8),A.LimitDate,112) as LimitDate
                                    FROM CaseMaster AS A  WITH (NOLOCK) 
                                    INNER JOIN CaseSendSetting AS B  WITH (NOLOCK) ON A.CaseId = B.CaseId
                                    LEFT OUTER JOIN [V_AgentAndDept] AS C  WITH (NOLOCK) ON C.EmpID = A.AgentUser
                                    LEFT OUTER JOIN [V_AgentAndDept] AS D  WITH (NOLOCK) ON D.EmpID = A.ApproveUser
                                    LEFT OUTER JOIN [V_AgentAndDept] AS E  WITH (NOLOCK) ON E.EmpID = A.ApproveUser2
									LEFT OUTER JOIN CaseSendSettingDetails AS F  WITH (NOLOCK) ON B.SerialID = F.SerialID
									LEFT OUTER JOIN MailInfo AS M  WITH (NOLOCK) on F.DetailsId = M.SendDetailId
