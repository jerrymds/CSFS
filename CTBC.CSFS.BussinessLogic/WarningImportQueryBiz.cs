...
{
    public class WarningImportQueryBiz : CommonBIZ
    {
        ...
        public bool GetImportTime(string executedDate, string timeSection)
        {
            try
            {
                string sql = @"select count(*) from Record_WarningImportQuery where ExecutedDate=@executedDate and TimeSection=@timeSection";
        ...
        public int InsertCaseMast(CaseMaster caseMast)
        {
            try
            {
                ...
                string PreSubDate = string.Empty;
                if (!string.IsNullOrEmpty(caseMast.PreSubDate))
                {
                    var twDate = caseMast.PreSubDate.Substring(0, 3) + "/" + caseMast.PreSubDate.Substring(3, 2) + "/" + caseMast.PreSubDate.Substring(5, 2);
                    PreSubDate = UtlString.FormatDateTwStringToAd(twDate);
                }
        ...
        //public int WarningImportQueryData(WarningImportQueryData WarningImportQueryData)
        //{
        //    try
        //    {
        //        int rtn = 0;
        //        string sql = @"insert into WarningImportQueryData(ExecutedDate,Timesection,DocNo,CaseNo,GovUnit,GovNo,CreatedDate,Added,GovDate) values(getdate(),@Timesection,@DocNo,@CaseNo,@GovUnit,@GovNo,getdate(),@Added,@GovDate)";
        //        base.Parameter.Clear();
        //        base.Parameter.Add(new CommandParameter("@Timesection", WarningImportQueryData.Timesection));
        //        base.Parameter.Add(new CommandParameter("@DocNo", WarningImportQueryData.DocNo));
        //        base.Parameter.Add(new CommandParameter("@CaseNo", WarningImportQueryData.CaseNo));
        //        base.Parameter.Add(new CommandParameter("@GovUnit", WarningImportQueryData.GovUnit));
        //        base.Parameter.Add(new CommandParameter("@GovNo", WarningImportQueryData.GovNo));
        //        base.Parameter.Add(new CommandParameter("@Added", WarningImportQueryData.Added));
        //        base.Parameter.Add(new CommandParameter("@GovDate", WarningImportQueryData.GovDate));
        //        rtn = base.ExecuteNonQuery(sql);
        //        return rtn;
        //    }
        //    catch (Exception ex)
        ...
        //public int WarningImportQueryData(WarningImportQueryData WarningImportQueryData, ref string Message)
        //{
        //    try
        //    {
        //        int rtn = 0;
        ...
        public int InsertRecord_WarningImportQuery(string timesection)
        {
            try
            {
                int rtn = 0;
                string sql = @"insert into Record_WarningImportQuery values(getdate(),@TimeSection)";
                base.Parameter.Clear();
                base.Parameter.Add(new CommandParameter("@TimeSection", timesection));
                rtn = base.ExecuteNonQuery(sql);
                return rtn;
        ...
        public IList<WarningImportQueryData> GetWarningImportQueryData(WarningImportQueryData model, int pageIndex, string strSortExpression, string strSortDirection, string EmpId)
        {
            try
            {
                string sqlWhere = "";
                ...
                if (!string.IsNullOrEmpty(model.COL_ID))
                {
                    sqlWhere += @" and COL_ID like @COL_ID ";
                    Parameter.Add(new CommandParameter("@COL_ID", "%" + model.COL_ID.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(model.COL_PID))
                {
                    sqlWhere += @" and COL_PID like @COL_PID ";
                    Parameter.Add(new CommandParameter("@COL_PID", "%" + model.COL_PID.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(model.COL_ACCOUNT2))
                {
                    sqlWhere += @" and GovNo like @COL_ACCOUNT2 ";
                    Parameter.Add(new CommandParameter("@COL_ACCOUNT2", "%" + model.COL_ACCOUNT2.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(model.COL_165CASE))
                {
                    sqlWhere += @" and COL_165CASE like @COL_165CASE ";
                    Parameter.Add(new CommandParameter("@COL_165CASE", "%" + model.COL_165CASE.Trim() + "%"));
                }
                if (!string.IsNullOrEmpty(model.ExecutedDateS))
                {
                    sqlWhere += @" AND CreatedDate >= @ExecutedDateS";
                ...
                if (!string.IsNullOrEmpty(model.ExecutedDateE))
                {
                    ...
                    sqlWhere += @" AND CreatedDate < @ExecutedDateE ";
                ...
                string strSql = @";with T1 
	                        as
	                        (
		                       select CONVERT(varchar(100),CreatedDate, 111) as CreatedDate,CONVERT(varchar(100),CreatedDate, 108)  as Timesection,COL_ID,COL_PID,COL_ACCOUNT,COL_165CASE,(select top 1 CodeDesc from ParmCode where Codetype = 'CaseCustStatus' and codeno = Status) as statusname  from WarningImport  WHERE 1=1 " + sqlWhere + @" 
	                        ),T2 as
	                        (
		                        select *, row_number() over (order by CreatedDate ASC , Col_ID ASC ) RowNum
		                        from T1
	                        ),T3 as 
	                        (
                ...
                IList<WarningImportQueryData> ilst = SearchList<WarningImportQueryData>(strSql);
                if (ilst != null)
                {
                    if (ilst.Count > 0)
                    {
                    ...
                    {
                        DataRecords = 0;
                        ilst = new List<WarningImportQueryData>();
                    }
                    return ilst;
                }
                return new List<WarningImportQueryData>();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public bool GetWarningImportQueryDataByFileName(string filename)
        {
            try
            {
  ...
  FROM [CaseEdocFile] where filename = '"+ filename+"')";
       ...
       public int DeleteAttatch(Guid CaseId, string fName)
        {
            string strSql = " delete from CaseEdocFile where CaseId=@CaseId and FileName=@FileName";
            base.Parameter.Clear();
        ...
        public List<CaseEdocFile> GetCaseEdocFileList(Guid CaseId, IDbTransaction trans = null)
        {
            string strSql = "select * from CaseEdocFile where CaseId=@CaseId";
            base.Parameter.Clear();
            base.Parameter.Add(new CommandParameter("@CaseId", CaseId));
            try
            {
                return trans == null ? base.SearchList<CaseEdocFile>(strSql).ToList() : base.SearchList<CaseEdocFile>(strSql, trans).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
