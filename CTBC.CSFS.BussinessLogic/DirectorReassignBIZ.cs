...
namespace CTBC.CSFS.BussinessLogic
{
    public class DirectorReassignBIZ : CommonBIZ
    {
            ...
            try
            {
                ...
                string strSql = @";with T1 
	                        ...
	                        (
                                ...
                                FROM CaseMaster A WITH (NOLOCK) 
                                left join CaseObligor B WITH (NOLOCK)  on A.CaseId=b.CaseId left join CaseSendSetting S WITH (NOLOCK) on A.CaseId=S.CaseId where Status not like '%Z%' and Status <> @StatusMiddle " + sqlWhere + @" 
	                        ),T2 as
	                        (
        ...
        public IList<CaseQuery> GetStatusName(string colName)
        {
            try
            {
                string strSql = @" select CodeDesc,CodeNo from PARMCode WITH (NOLOCK) where CodeType=@CodeType ORDER BY SortOrder";
                base.Parameter.Clear();
        ...
        public int checkAgentDepartment(string DepartmentID, string EmpID)
        {
           try
           {
              string strSql = @" select count(0) from LDAPEmployee WITH (NOLOCK) where DepDN like @DepartmentID and EmpID=@EmpID";
              base.Parameter.Clear();
