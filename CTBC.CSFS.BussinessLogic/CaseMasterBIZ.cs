    ...
    public class CaseMasterBIZ : CommonBIZ
    {
        ...
        public string GetAutoEmployee(string Ckd, bool IsAutoDispatch, bool IsAutoDispatchFS)
        {
            ...
            if (IsAutoDispatch && Ckd == CaseKind.CASE_SEIZURE)//扣押案件
            {
                ...
                strSql = @"select count(*) from CaseMaster where CaseNo like 'A%' and CreatedDate between convert(nvarchar(10),DATEADD(DAY,-60, GETDATE()),23) and getdate();";
        ...
        public void updateCaseMasterAgentUser(Guid caseid, string eQueryStaff)
        {
        ...
        public void setCaseMasterRunning(List<Guid> caseids, string Status)
        {
        ...
        public IList<CaseMaster> getCaseMaster(string Type, string Status, string CaseNo = null)
        {
            string strsql = @"SELECT c.*  FROM [dbo].[CaseMaster] c inner join [dbo].EDocTXT3 e on c.CaseId=e.CaseId 
   where CaseKind2=@Type and ReceiveKind='電子公文' and c.Status=@Status and IsEnable='1' and NULLIF(ReturnReason, '') IS NULL  ";
            ...
            return SearchList<CaseMaster>(strsql);
        }


        public string getAgentUser(Guid caseid)
        {
            string strsql = @"SELECT * FROM [dbo].[CaseMaster] where caseid=@CaseID";

            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseID", caseid));
            var result = SearchList<CaseMaster>(strsql);
            if (result != null)
            {
                return result.First().AgentUser;
            }
            else
            {
                return "";
        ...
        /// <returns></returns>
        public IList<CaseMaster> getCaseMasterByGovNo(string txtGovNo, string txtPreSubDate, List<CaseObligor> caseObligorsList)
        {
            try
            {
                List<CaseMaster> AllResult = new List<CaseMaster>();
                DateTime dt = DateTime.Parse(txtPreSubDate);
                string strSql = "select * from CaseMaster where  GovNo=@GovNo and GovDate=@GovDate";
                Parameter.Clear();
                Parameter.Add(new CommandParameter("@GovNo", txtGovNo));
                Parameter.Add(new CommandParameter("@GovDate", dt));
                ...
                {
                    var cos = coBiz.GetObligorsList(c.CaseId).ToList();
                    bool isAllMatch = true;
                    foreach (var c1 in cos)
                    {
                        if (!caseObligorsList.Any(x => x.ObligorNo == c1.ObligorNo))
                        {
                            isAllMatch = isAllMatch & false;
                        }
                    }
                    if (isAllMatch)
                        AllResult.Add(c);
            ...
            try
            {
                using (dbConnection)
                {
					...
					foreach (CaseObligor obligor in model.CaseObligorlistO)
					{
                        ...
                        if( ! string.IsNullOrEmpty(obligor.ObligorNo))
                        //if (obligor.ObligorNo != null)
						{
							string strUp = obligor.ObligorNo.Trim();
							obligor.ObligorNo = strUp.ToUpper();
                    ...
                    if ((oldMaster.Receiver != model.CaseMaster.Receiver) && (model.CaseMaster.CaseKind2 == "扣押" || model.CaseMaster.CaseKind2 == "扣押並支付"))//受文者，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                    ...
                    if ((oldMaster.ReceiveAmount != model.CaseMaster.ReceiveAmount)  && ( model.CaseMaster.CaseKind2 == "扣押" || model.CaseMaster.CaseKind2 == "扣押並支付"))//來函扣押總金額，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                    ...
                    if ((oldMaster.NotSeizureAmount != model.CaseMaster.NotSeizureAmount)  && (model.CaseMaster.CaseKind2 == "扣押" || model.CaseMaster.CaseKind2 == "扣押並支付"))//金額未達毋需扣押，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                    ...
                    // adam 20200722
                    if (oldMaster.PreSubAmount != model.CaseMaster.PreSubAmount && model.CaseMaster.CaseKind2 == "支付" )//前案扣押金額，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                        ...
                        log.ColumnValueAfter = model.CaseMaster.PreSubAmount.ToString();
                        log.TabID = "Tab1-2";
                        log.TabName = "公文資訊-案件資訊";
                        log.TableName = "CaseMaster";
                        log.DispSrNo = 9;
                    ...
                    if (oldMaster.PreReceiveAmount != model.CaseMaster.PreReceiveAmount && model.CaseMaster.CaseKind2 == "支付" )//收取金額，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                        ...
                        log.ColumnValueAfter = model.CaseMaster.PreReceiveAmount.ToString();
                        log.TabID = "Tab1-2";
                        log.TabName = "公文資訊-案件資訊";
                        log.TableName = "CaseMaster";
                        log.DispSrNo = 9;
                    ...
                    if (oldMaster.OverCancel != model.CaseMaster.OverCancel && model.CaseMaster.CaseKind2 == "支付")//超過收取金額部份是否撤銷，有修改，新增CaseDataLog
                    {
                        log.TXSNO = TXSNO;
                        log.TXDateTime = TXDateTime;
                        log.TXType = "修改";
                        ...
                        log.ColumnValueAfter = model.CaseMaster.OverCancel.ToString();
                        log.TabID = "Tab1-2";
                        log.TabName = "公文資訊-案件資訊";
                        log.TableName = "CaseMaster";
                        log.DispSrNo = 9;
        ...
        public DateTime GetPayDate(string caseKind2, DateTime createDate)
        {
            ...
            DateTime payDate = caseKind2 == CaseKind2.CaseSeizureAndPay
                ...
                : UtlString.GetCheckDate(createDate, checkDateArray[0], checkDateArray[1], checkDateArray[2]);
            return payDate;
        }
        ...
        public bool UpdateCaseMasterStatus(Guid caseid, string Status, string userId, string returnReason)
        {
            string strSql = string.Empty;
            Parameter.Clear();
            if (Status == "D01")
            {
                strSql = @"UPDATE [CaseMaster] SET [Status] = @Status,[ReturnReason]=@ReturnReason,
        ...
        public bool UpdateCaseMasterAddCharge(Guid caseid,int AddCharge)
        {
            string strSql = string.Empty;
            Parameter.Clear();

            strSql = @"UPDATE [CaseMaster] SET [AddCharge] = @AddCharge WHERE [CaseId] = @CaseId";
        ...
        /// <returns></returns>
        public DataTable GetCaseMasterByDocNo(string docno)
        {
            string strsql = @"SELECT *  FROM  CaseMaster   WHERE  CaseNo = @DocNo" ;
            Parameter.Clear();
        ...
        public IList<CaseMaster> GetQueryList(CaseMaster model, int pageIndex, string strSortExpression, string strSortDirection, ref string strsqlWhere)
        {
            try
            {
                ...
                //base.Parameter.Add(new CommandParameter("@pageE", base.PageSize * base.PageIndex));
                base.Parameter.Clear();
                base.Parameter.Add(new CommandParameter("@pageS", (300 * (base.PageIndex - 1)) + 1));
                base.Parameter.Add(new CommandParameter("@pageE", 300 * base.PageIndex));
                ...
                if (model.PostType == "1") //支付類
                {
                    ...
                    sqlStr += @" select  a.CaseId,a.DetailsId,
                                ...
                                where (css.Template='支付' or css.Template='支付電子回文') and (a.SendType = 1 or (a.SendType = 2 and css.SendKind <> '電子發文'))  and  (m.Status=@Status1)  " + sqlWhere;// +@"
                                //where (css.Template='支付' or css.Template='支付電子回文')  and  (m.Status=@Status1)  " + sqlWhere;// +@"
                ...
                if (model.PostType == "3")  //死亡案件
                {
                    sqlStr += @" select  ROW_NUMBER() OVER (ORDER BY GovName) AS Sno,a.CaseId,a.DetailsId,
                                 a.SendType,A.GovName,B.MailNo -- Right('00000000000000' + Cast(STR(B.MailNo, LEN(B.MailNo), 0) as varchar),14)  as MailNo
                                ,CONVERT(varchar(100),b.MailDate,111) as MailDate,b.CreatedUser, 
                                css.SendWord,css.SendNo,css.SendDate,
                                m.DocNo,m.CaseNo,m.CaseKind,m.CaseKind2,m.AgentUser
                                from CaseSendSettingDetails as A
                                LEFT  JOIN MAILINFO AS B ON A.CaseId=B.CaseId AND A.DetailsId=B.SendDetailId
                                LEFT  JOIN CaseSendSetting as css on A.CaseId=css.CaseId and a.SerialID=css.SerialID
                                LEFT  JOIN CaseMaster as M ON A.CaseId=m.CaseId 
                                where (css.Template='國稅局死亡' )  and  (m.Status=@Status1 or m.Status=@Status2) " + sqlWhere;// +" order by AgentUser,CaseNo,SendType asc ";
                    strSql = @";with T1 
	                        as
	                        (
                               " + sqlStr + "),";
                    strSql += @"
	                        ...
	                        select a.* from T3 a order by a.SendNo ";
                }
                /// adam
                /// 
                ///
        ...
        #region 設定掛號號碼
        public int CreateMailNo(CaseMaster model, string[] aryId, string userId ,string strType)
        {
            /// adam
            int intType = 1;
            string sno = "0";
            ...
            try
            {
                using (dbConnection)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        {
                            case 1:
                                if (strMailNo.Length < 14)
                                {
                                    strMailNo = strMailNo.PadLeft(14, '0');
                                    serial6 = strMailNo.Substring(0, 6);
                                }
                                else
                                {
                                    serial6 = strMailNo.Substring(0, 6);
                                }
                                string strNum1 = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                strMailNo2 = strNum1 + strMailNo.Substring(6, 8);
                                ...
                                {
                                    Create(item.CaseId, item.DetailsId.ToString(), userId, strMailNo2, model.MailDate, dbTransaction);
                                    strGovName = item.GovName;
                                    strGovAddr = item.GovAddr;
                                    strMailNo = Convert.ToString(Convert.ToInt32(strNum1) + 1).PadLeft(6, '0') + strMailNo.Substring(6, 8);//地址不同時,將mailNo加1
                                    //strMailNo = Convert.ToString(Convert.ToInt32(strMailNo) + 1);//地址不同時,將mailNo加1                                   
                                }
                                else
                                {
                                    if (rowrelation1[0]["MailNo"].ToString().Length < 14)
                                    {
                                        strMailNo = rowrelation1[0]["MailNo"].ToString().PadLeft(14, '0');
                                        serial6 = strMailNo.Substring(0, 6);
                                    }
                                    else
                                    {
                                        serial6 = rowrelation1[0]["MailNo"].ToString().Substring(0, 6);
                                    }
                                    string oldNum1 = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                    strMailNo1 = oldNum1 + strMailNo.Substring(6, 8);
                            ...
                            case 2:
                                ...
                                string strNum2 = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                strMailNo2 = strNum2 + strMailNo.Substring(6, 8);
                                ...
                                //if (item.GovName != strGovName || item.GovAddr != strGovAddr)//第一次如果沒有出現重複的時候
                                {
                                    ...
                                    strGovName = item.GovName;
                                    strGovAddr = item.GovAddr;
                                    strMailNo = Convert.ToString(Convert.ToInt32(strNum2) + 1).PadLeft(6, '0') + strMailNo.Substring(6, 8);//地址不同時,將mailNo加1
                                    //strMailNo = Convert.ToString(Convert.ToInt32(strMailNo) + 1);//地址不同時,將mailNo加1
                                }
                                else
                                {
                                    if (strSendType == "2")
                                    {
                                        if (rowrelation2[0]["MailNo"].ToString().Length < 14)
                                        {
                                            strMailNo = rowrelation2[0]["MailNo"].ToString().PadLeft(14, '0');
                                            serial6 = strMailNo.Substring(0, 6);
                                        }
                                        else
                                        {
                                            serial6 = rowrelation2[0]["MailNo"].ToString().Substring(0, 6);
                                        }
                                        string oldNum2 = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                        strMailNo1 = oldNum2 + strMailNo.Substring(6, 8);
                            ...
                            case 3:
                                ...
                                string strNum3 = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                strMailNo2 = strNum3 + strMailNo.Substring(6, 8);
                                Create(item.CaseId, item.DetailsId.ToString(), userId, strMailNo2, model.MailDate, dbTransaction);
                                strMailNo2 = strMailNo;
                                strGovName = item.GovName;
                                strGovAddr = item.GovAddr;
                                strMailNo = Convert.ToString(Convert.ToInt32(strNum3) + 1).PadLeft(6,'0') + strMailNo.Substring(6, 8);//地址不同時,將mailNo加1
                                //strMailNo = Convert.ToString(Convert.ToInt32(strMailNo) + 1);//地址不同時,將mailNo加1
                            ...
                            case 4:
                                ...
                                string strNum = Convert.ToUInt32(serial6).ToString().PadLeft(6, '0');
                                strMailNo2 = strNum + strMailNo.Substring(6, 8);
                                Create(item.CaseId, item.DetailsId.ToString(), userId, strMailNo2, model.MailDate, dbTransaction);
                                //strMailNo2 = strMailNo;
                                strGovName = item.GovName;
                                strGovAddr = item.GovAddr;
                                strMailNo = Convert.ToString(Convert.ToInt32(strNum) + 1).PadLeft(6, '0') + strMailNo.Substring(6, 8);//地址不同時,將mailNo加1
                                //strMailNo = Convert.ToString(Convert.ToInt32(strMailNo) + 1);//地址不同時,將mailNo加1
                                DataRow drGovAddress4 = dtGovAddress.NewRow();
                                drGovAddress4["GovName"] = item.GovName;
                                drGovAddress4["GovAddr"] = item.GovAddr;
                                drGovAddress4["MailNo"] = strMailNo2;
                                dtGovAddress.Rows.Add(drGovAddress4);
                                break;
                        }
                    }
                    dbTransaction.Commit();
        ...
        public MemoryStream MailNoExcel_NPOI(string TemplatePath, string postType, string strSqlWhere, string strMailNo1, string strMailNo2, string userId)
        {
            ...
            if (queryList != null && queryList.Count > 0)
            {
                if (queryList.Count <= 20)
                {
                    ...
                    {
                        if (item.mailno.MailNo.Length > 5)
                        { 
                           SetExcelCell(sheet, r, 2, sourceRow.Cells[2].CellStyle, item.mailno.MailNo.PadLeft(14,'0'));
                        }
                        else
                        {
        ...
        #region 列印地址條
        public DataTable AddrSearchList(string postType, string strMailNo1, string strMailNo2, string inMailNo, string inDetailsId)
        {
            string strSql = "";
            string sqlWhere = "";
            if (!string.IsNullOrEmpty(strMailNo1))
            ...
            if (!string.IsNullOrEmpty(inMailNo))
            {
                sqlWhere += " and MailNo in (" + inMailNo + ") and d.DetailsId in (" + inDetailsId+ ") ";
            }
...
and c.CreatedDate<=CONVERT(varchar(10), DateAdd(d,-1,getdate()), 23)  ";
        ...
        {
            string rtnPayDate = "";
            string strSql = @"select CONVERT(varchar(100) , [PayDate], 105) as [PayDate] from CaseMaster where CaseId=@CaseId";
            Parameter.Clear();
            Parameter.Add(new CommandParameter("@CaseId", caseId));
            IList<CaseMaster> list = SearchList<CaseMaster>(strSql);
            if (list != null)
            {
