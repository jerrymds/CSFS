...
{
    public partial class Htg87016_Old : HtgXmlPara
    {
    ...
    public partial class Htg87016 : HtgXmlPara
    {
        ...
        public override DataSet TransferXmlToDataSet(ArrayList recxmldatalist, out string strmessage)
        {
            ...
            try
            {
                ...
                // 20200924, 先找Type = 'S'(匯款) 的列出來, 排除'C' (現金)的...
                ...
                for (int i = 0; i < recxmldatalist.Count; i++)
                {
                    ...
                    #region 若為第一頁則要insert
                    {
                        ...
                        for(int ii=1;ii<=25;ii++)
                        {
                            ...
                            decimal buyrate = 1.0m;
                            decimal sellrate = 1.0m;
                            string typeNo = "TYPE_" + ii.ToString("0#");

                            XmlNode typeXML = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='" + typeNo + "']");


                            if (typeXML != null)
                            {
                                //sellrate = (decimal)(Int64.Parse(sell.Attributes["value"].Value) / 10000000.0);
                                if( typeXML.Attributes["value"].Value.ToString().ToUpper().Equals("S"))
                                {
                                    DataRow _dr = dt87016.NewRow();    
                                    string noStr = "CCYCODE_FROM_" + ii.ToString();
                                    XmlNode ccyXML = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='" + noStr + "']");
                                    string _ccy = ccyXML.Attributes["value"].Value.ToString().ToUpper();

                                    XmlNode buyXML = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='BUY_RATE_" + ii.ToString() + "']");
                                    buyrate = (decimal)(Int64.Parse(buyXML.Attributes["value"].Value) / 10000000.0);

                                    XmlNode sellXML = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='SELL_RATE_" + ii.ToString() + "']");
                                    sellrate = (decimal)(Int64.Parse(sellXML.Attributes["value"].Value) / 10000000.0);
