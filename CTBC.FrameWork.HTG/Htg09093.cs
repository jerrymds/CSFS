...
{
    public partial class Htg09093 : HtgXmlPara
    {
        #region 全域變數
            ...
            string strxml = @"<?xml version=""1.0""?>
                                <hostgateway>
                                  <header>
                                    ...
                                    <data id=""transactionId"" value=""009093""/>
                                  ...
                                  <body>
                                    ...
                                    <data value=""66"" id=""STOP_REASON_NO""/>
                                    <data value=""S"" id=""AMOUNT""/>
                                    <data value=""S"" id=""HOLD_RESN_DESC""/>
                                    <data value="""" id=""PRMO_NO""/>
                                    <data value=""0000000000000000"" id=""LOAN_ACCT_NO""/>
                                    <data value=""ccy"" id=""CURRENCY""/>
                                    <data value="""" id=""NO_RECEIPT""/>
                                    <data value=""00000000"" id=""PRESET_REMOVAL_DT""/>
        ...
        public override string GetPostTransactionXml(string recxml, Hashtable parmht)
        {
            ...
            datanode.Attributes["value"].Value = "009093";
        ...
        public override DataSet TransferXmlToDataSet(ArrayList recxmldatalist, out string strmessage)
        {
            ...
            DataTable dt09093 = Createdt09093();
            ...
            Dictionary<string, string> masterMapping = new Dictionary<string, string>() { 
                {"Account","Account"},{"RESPONSE_OC08","RESPONSE_OC08"},{"CURRENCY_OC08","CURRENCY_OC08"},{"CMTRASH_OC08","CMTRASH_OC08"},{"DESCRIP_OC08","DESCRIP_OC08"},{"RAMOUNT_OC57","RAMOUNT_OC57"},{"ACCT_NO_OC57","ACCT_NO_OC57"},{"ACCNAME_OC57","ACCNAME_OC57"},{"RDATE_OC57","RDATE_OC57"},{"outputCode","outputCode"}
            };
            ...
            try
            {
                ...
                for (int i = 0; i < recxmldatalist.Count; i++)
                {
                    ...
                    #region 若為第一頁則要insert dt09093
                    if (linedata.Count >1)
                    //if (i == 0) // 表示第一頁
                    {
                        //寫入dt09093
                        DataRow _dr = dt09093.NewRow();
                        for (int line = 2; line <= linedata.Count; line++) // 因為有時會在LINE=3 , 有時在LINE=4.. 所以就全掃描
                        {
                            var str = "/hostgateway/line[@no='" + line.ToString() + "']/msgBody/RESPONSE_OC08";
                            var qqq = xmldoc.SelectSingleNode(str);
                            //var hasElment = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='RESPONSE_OC08']");
                            //var hasOC57 = xmldoc.SelectSingleNode("/hostgateway/line[@no='" + line.ToString() + "']/msgBody/data[@id='RAMOUNT_OC57']");  
                            
                            //if (hasElment == null || hasOC57==null)
                            //    continue;
        ...
        private DataTable Createdt09093()
        {
            DataTable dt = new DataTable("TX_09093");
            ...
            dt.Columns.Add(new DataColumn("RAMOUNT_OC57"));
            dt.Columns.Add(new DataColumn("ACCT_NO_OC57"));
            dt.Columns.Add(new DataColumn("ACCNAME_OC57"));
            dt.Columns.Add(new DataColumn("RDATE_OC57"));
